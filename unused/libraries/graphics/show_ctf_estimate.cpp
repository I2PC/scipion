...
AssignCTFViewer::AssignCTFViewer(const FileName &_fn_psd,
                                 ProgCTFEstimateFromMicrograph &_assign_ctf_prm):
        ImageViewer(_fn_psd.c_str(), false)
{
    // Get input parameters .................................................
    assign_ctf_prm = _assign_ctf_prm;
    fn_psd = _fn_psd;
    if (assign_ctf_prm.prmEstimateCTFFromPSD.initial_ctfmodel.K == 0)
        assign_ctf_prm.prmEstimateCTFFromPSD.initial_ctfmodel.K = 1;
    ...
    initial_value.push_back(assign_ctf_prm.prmEstimateCTFFromPSD.min_freq*100);
    // Set max_freq
    min.push_back(0);
    max.push_back(50);
    prm_name.push_back("Maximum freq. (x100)");
    initial_value.push_back(assign_ctf_prm.prmEstimateCTFFromPSD.max_freq*100);
    // Set DefocusU
    min.push_back(-100);
    max.push_back(-1);
    prm_name.push_back("DefocusU (/1000)");
    initial_value.push_back(
        assign_ctf_prm.prmEstimateCTFFromPSD.initial_ctfmodel.DeltafU / 1000);
    // Set DefocusV
    min.push_back(-100);
    max.push_back(-1);
    prm_name.push_back("DefocusV (/1000)");
    initial_value.push_back(
        assign_ctf_prm.prmEstimateCTFFromPSD.initial_ctfmodel.DeltafV / 1000);
    // Set Angle
    min.push_back(0);
    max.push_back(360);
    prm_name.push_back("Angle");
    initial_value.push_back(
        assign_ctf_prm.prmEstimateCTFFromPSD.initial_ctfmodel.azimuthal_angle);
...
void AssignCTFViewer::drawFirstZero(std::vector<float> &prm)
{
    ...
    ctfmodel = assign_ctf_prm.prmEstimateCTFFromPSD.initial_ctfmodel;
...
void AssignCTFViewer::okToProceed()
{
    ...
    ;
    if (!assign_ctf_prm.prmEstimateCTFFromPSD.modelSimplification)
    ...
    fh_adjust_param
    << "voltage=             " << assign_ctf_prm.prmEstimateCTFFromPSD.initial_ctfmodel.kV << std::endl
    << "spherical_aberration=" << assign_ctf_prm.prmEstimateCTFFromPSD.initial_ctfmodel.Cs << std::endl
    << "sampling_rate=       " << assign_ctf_prm.prmEstimateCTFFromPSD.initial_ctfmodel.Tm << std::endl
    << "defocusU=            " << current_prm[2]*1000              << std::endl
    << "defocusV=            " << current_prm[3]*1000              << std::endl
    << "azimuthal_angle=     " << current_prm[4]                   << std::endl
    << "ctfmodelSize=        " << assign_ctf_prm.prmEstimateCTFFromPSD.ctfmodelSize      << std::endl
    ;
