...
"""%(DIMENSION,DIMENSION,NUMIMAGES)
    ...
    def test_cudaRotateImage(self):
        ...
        #transform projections (rotation, shift, scale, and flip)
        command = "xmipp_cuda_rotate_image "
        ...
        if USE_MD!="md":
            ...
            args += " --shift %d %d"%(SHIFT_X, SHIFT_Y)
            args += " --scale %d"%SCALE_IM
            if FLIP:
                args += " --flip"
        runJob(None, command, args,env=getEnviron())
