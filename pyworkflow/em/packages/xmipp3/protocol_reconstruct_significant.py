...
class XmippProtReconstructSignificant(ProtInitialVolume):
    ...
    def _defineParams(self, form):
        ...
        form.addParam('useImed', BooleanParam, default=True,
                      expertLevel=LEVEL_ADVANCED,
                      label='Use IMED',
                      help='Use IMED for the weighting. IMED is an alternative to correlation that can '
                           'discriminate better among very similar images')
        form.addParam('strictDir', BooleanParam, default=False,
                      expertLevel=LEVEL_ADVANCED,
                      label='Strict direction',
                      help='If the direction is strict, then only the most significant experimental images '
        ...
        form.addParam('angDistance', IntParam, default=10,
                      ...
                      help='Images in an angular neighborhood also determines the weight of each image. '
        ...
        form.addParam('dontApplyFisher', BooleanParam, default=False,
                      ...
                      help="Images are preselected using Fisher's confidence interval on the correlation "
    ...
    def significantStep(self, iterNumber, alpha, args):
        ...
        self.runJob("xmipp_transform_mirror",
                    "-i %s -o %s --flipX" % (volFn, fnSym), numberOfMpi=1)
        self.runJob("xmipp_transform_mirror", "-i %s --flipY" % fnSym,
                    numberOfMpi=1)
        self.runJob("xmipp_transform_mirror", "-i %s --flipZ" % fnSym,
                    numberOfMpi=1)
        self.runJob("xmipp_image_operate", "-i %s --plus %s" % (fnSym, volFn),
                    numberOfMpi=1)
    ...
    def _validate(self):
        ...
        if self.thereisRefVolume:
            if self.refVolume.hasValue():
                ...
                if x1 != x2 or y1 != y2:
                    errors.append(
                        'The input images and the reference volume have different sizes')
            else:
        ...
        if (100 - self.alpha0.get()) / 100.0 * (SL.getTrueSymsNo() + 1) > 1:
            errors.append(
                "Increase the initial significance it is too low for this symmetry")
        return errors
    ...
    def _methods(self):
        ...
        if self.inputSet.get() is not None:
            ...
            retval += "from the set of classes %s." % self.getObjectTag(
                'inputSet')
            if self.thereisRefVolume:
            ...
            if self.hasAttribute('outputVolume'):
                retval += " The reconstructed volume was %s." % self.getObjectTag(
                    'outputVolume')
        return [retval]
    ...
    def getLastIteration(self, Nvolumes):
        ...
        for n in range(1, self.iter.get() + 1):
            NvolumesIter = len(
                glob(self._getExtraPath('volume_iter%03d*.vol' % n)))
            if NvolumesIter == 0:
                continue
