...
class XmippGeometricalMask3D:
    ...
    def defineParams(self, form, isGeometry, addSize):
        ...
        form.addParam('geo', EnumParam, label='Mask type', default=xc.MASK3D_SPHERE, condition=isGeometry, 
                      choices = ['Sphere', 'Box', 'Crown', 'Cylinder', 
                                 'Gaussian', 'Raised cosine', 'Raised crown'])
        # TODO add wizard
        form.addParam('radius', IntParam, default=-1, 
                      condition='(geo==%d or geo==%d) and %s' % (xc.MASK3D_SPHERE, xc.MASK3D_CYLINDER, isGeometry),
                      label="Radius (px)", help="Mask radius, if -1, the radius will be MaskSize/2")
        form.addParam('boxSize', IntParam, default=-1, condition='geo==%d and %s' % (xc.MASK3D_BOX,isGeometry),
                      label="Box size", help="Mask box size, if -1, the box size will be MaskSize/2")
        radiusCondition = '(geo==%d or geo==%d or geo==%d) and %s' % (xc.MASK3D_CROWN, xc.MASK3D_RAISED_COSINE, xc.MASK3D_RAISED_CROWN, isGeometry)
        # TODO add wizard
        ...
        form.addParam('height', IntParam, default=-1, condition='geo==%d and %s' % (xc.MASK3D_CYLINDER,isGeometry), 
                      label="Height (px)", help="Cylinder height in pixels. If -1, height will be MaskSize")
        form.addParam('sigma', FloatParam, default=-1, condition='geo==%d and %s' % (xc.MASK3D_GAUSSIAN,isGeometry),
                      label="Sigma (px)", help="Gaussian sigma in pixels. If -1, sigma will be MaskSize/6")                
        form.addParam('borderDecay', IntParam, default=0, condition='geo==%d and %s' % (xc.MASK3D_RAISED_CROWN,isGeometry),
    ...
    def argsForTransformMask(self,size):
        ...
        if geo == xc.MASK3D_SPHERE:
            args += 'circular %d' % r
        elif geo == xc.MASK3D_BOX:
            b = self.boxSize.get()
            if b==-1:
                b = size/2
            args += 'rectangular %d %d %d' % (-b, -b, -b)
        elif geo == xc.MASK3D_CROWN:
            args += 'crown %d %d' % (iR, oR)
        elif geo == xc.MASK3D_CYLINDER:
            height = self.height.get()
            if height==-1:
                height = size
            args += 'cylinder %d %d' % (r, -height)
        elif geo == xc.MASK3D_GAUSSIAN:
            sigma = self.sigma.get()
            if sigma==-1:
                sigma = size/6.0
            args += 'gaussian %f' % -sigma                        
        elif geo == xc.MASK3D_RAISED_COSINE:
            args += 'raised_cosine %d %d' % (iR, oR)
        elif geo == xc.MASK3D_RAISED_CROWN:
...
class XmippGeometricalMask2D:
    ...
    def defineParams(self, form, isGeometry, addSize):
        ...
        form.addParam('geo', EnumParam, label='Mask type', default=xc.MASK2D_CIRCULAR, condition=isGeometry, 
                      choices = ['Circular', 'Box', 'Crown', 
                                 'Gaussian', 'Raised cosine', 'Raised crown'])
        # TODO add wizard
        form.addParam('radius', IntParam, default=-1, 
                      condition='geo==%d and %s' % (xc.MASK2D_CIRCULAR, isGeometry),
                      label="Radius (px)", help="Mask radius, if -1, the radius will be MaskSize/2")
        form.addParam('boxSize', IntParam, default=-1, condition='geo==%d and %s' % (xc.MASK2D_BOX, isGeometry),
                      label="Box size", help="Mask box size, if -1, the box size will be MaskSize/2")
        radiusCondition = '(geo==%d or geo==%d or geo==%d) and %s' % (xc.MASK2D_CROWN, xc.MASK2D_RAISED_COSINE, xc.MASK2D_RAISED_CROWN, isGeometry)
        # TODO add wizard
        form.addParam('innerRadius', IntParam, default=0, 
                      condition=radiusCondition,
                      label="Inner radius (px)", help="Inner radius in pixels")
        ...
        form.addParam('outerRadius', IntParam, default=-1,
                      condition=radiusCondition,
                      label="Outer radius (px)", help="Outer radius in pixels, if -1, the outer radius will be MaskSize/2")
        form.addParam('sigma', FloatParam, default=-1, condition='geo==%d and %s' % (xc.MASK2D_GAUSSIAN,isGeometry),
                      label="Sigma (px)", help="Gaussian sigma in pixels. If -1, sigma will be MaskSize/6")                
        form.addParam('borderDecay', IntParam, default=0, condition='geo==%d and %s' % (xc.MASK2D_RAISED_CROWN,isGeometry),
                      label="Border decay (px)", help="This is the fall-off of the two borders of the crown")        
    ...
    def argsForTransformMask(self,size):
        ...
        if geo == xc.MASK2D_CIRCULAR:
            args += 'circular %d' % r
        elif geo == xc.MASK2D_BOX:
            b = self.boxSize.get()
            if b==-1:
                b = size/2
            args += 'rectangular %d %d' % (-b, -b)
        elif geo == xc.MASK2D_CROWN:
            args += 'crown %d %d' % (iR, oR)
        elif geo == xc.MASK2D_GAUSSIAN:
            sigma = self.sigma.get()
            if sigma==-1:
                sigma = size/6.0
            args += 'gaussian %f' % -sigma                        
        elif geo == xc.MASK2D_RAISED_COSINE:
            args += 'raised_cosine %d %d' % (iR, oR)
        elif geo == xc.MASK2D_RAISED_CROWN:
            args += 'raised_crown %d %d %d' % (iR, oR, self.borderDecay.get())               
        else:
            raise Exception("No valid mask type value %d" % geo)
        return args
