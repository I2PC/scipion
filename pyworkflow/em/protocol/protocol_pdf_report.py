...
class ProtPDFReport(EMProtocol):
    """ 
    Produce a pdf report from the files (*.tex, *.txt, *.jpg, *.png, *.pdf) files in a directory.
    Files in the directory are sorted by name alphabetically, so if you want them to have the right order a possibility is to name them as
    0010-myText.txt, 0020-aFigure.png, 0030-anotherFigure.jpg, 0040-aPaper.pdf, 0050-anotherText.tex, ... when these files are sorted, they will
    be sorted by the number in front. 
    """    
    _label = 'pdf report'
    ...
    def _defineParams(self, form):
        ...
        form.addParam('filesPath', PathParam, default=os.environ.get('SCIPION_NOTES_ARGS', ""),
    ...
    def createReport(self):
        fnTex = self._getExtraPath("report.tex")
        fhTex = open(fnTex,"w")
        template ="""
...
"""
        ...
        for fileName in sorted(glob.glob(os.path.join(fnDir,"*"))):
            _, fileNameWithoutDir = os.path.split(fileName)
            fnDest = self._getExtraPath(fileNameWithoutDir)
            fnDest = fnDest.replace(" ","_")
            fnDest = fnDest.replace(":","_")
            fnDest = fnDest.replace(";","_")
            copyFile(fileName,fnDest)
...
"""
        ...
        fhTex.close()
        self.runJob("pdflatex", "-interaction=nonstopmode -output-directory %s %s"%(self._getExtraPath(),self._getExtraPath("report.tex")))
