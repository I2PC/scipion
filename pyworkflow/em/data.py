...
class Image(EMObject):
    ...
    def __init__(self, location=None, **kwargs):
        ...
        #in genral next  variable is used to relate subvolumes with original volumes
        # _coordianteSystem provides a matrix that relates the default system of
        # coordinantes (in which the origin is in the image center (xDim/2, yDim/2, [zDim/2])
        # and the axis are x -> the fastest changing index and z -> the slowest changing one)
        # with the one in which the image has been created.
        # _coordianteSystem is a matrix with four columns. In the first column
        # there are the coordinates of the default vector i (parallel to the x axis)
        # expressed in the creation system of coordinates, the same happens in the
        # second and third columns with respect to unit vectors j and k.
        # finally in the fourth column we have the origin of the canonical system
        # expressed in the user defined system.
        # this is an object of type em.Transform() that
        # contains a 4x4 np.array
        self._coordenateSystem = None
    ...
    def hasCoordenateSystem(self):
        return self._coordenateSystem is not None

    def getCoordenateSystem(self):
        return self._coordenateSystem

    def setCoordenateSystem(self, newTransform):
        self._coordenateSystem = newTransform
