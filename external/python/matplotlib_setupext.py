...
def check_for_tk():
	...
	xmipp_base = join(environ['XMIPP_HOME'], 'external', 'python')
	inc_dirs = [join(xmipp_base, '%s8.5.10' % k, 'generic') for k in ['tk', 'tcl']]	
        module = Extension('test', [], include_dirs=inc_dirs)
        try:
