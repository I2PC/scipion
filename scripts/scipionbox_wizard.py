...
class BoxWizardView(tk.Frame):
    ...
    def _fillContent(self, frame):
        ...
        _addCheckPair(OPTICAL_FLOW, 3, labelFrame2)
        _addCheckPair(SUMMOVIE, 3, labelFrame2)
        _addCheckPair(CTFFIND4, 5, labelFrame2)
        _addCheckPair(GCTF, 5, labelFrame2, col=2)
        _addPair("Monitors", 6, labelFrame2, entry=False)
        _addCheckPair(EMAIL_NOTIFICATION, 6, labelFrame2)
        _addCheckPair(HTML_REPORT, 6, labelFrame2, col=2)
    ...
    def _onAction(self, e=None):
        ...
        if backupFolder == '':
            ...
            errors.append(
                """Backup folder '%s' already exists. If you want to use you must delete it first""" % backupFolder)
