...
/* Read -------------------------------------------------------------------- */
void CTFDescription::readFromMetadataRow(const MetaData &md, size_t id,
...
void CTFDescription::defineParams(XmippProgram * program) {
	...
	program->addParamsLine(
			"  [--defocusU <DeltafU>]                : Defocus in Angstroms (Ex: 2000)");
	program->addParamsLine(
...
void CTFDescription::produceSideInfo() {
	...
	K3 = pow(0.25 * PI * local_Ca * lambda * (espr / kV + 2 * local_ispr), 2)
			/ log(2.0);
	K5 = PI * DeltaF * lambda;
...
void CTFDescription::precomputeValues(const MultidimArray<double> &cont_x_freq,
	...
	FOR_ALL_ELEMENTS_IN_ARRAY2D(cont_x_freq)
	{
		...
		if (fabs(X) < XMIPP_EQUAL_ACCURACY && fabs(Y) < XMIPP_EQUAL_ACCURACY)
...
/* Zero -------------------------------------------------------------------- */
//#define DEBUG
void CTFDescription::zero(int n, const Matrix1D<double> &u,
...
void CTFDescription::applyCTF(MultidimArray<std::complex<double> > &FFTI) {
	...
	if ( ZSIZE(FFTI) > 1)
		REPORT_ERROR(ERR_MULTIDIM_DIM,
				"ERROR: Apply_CTF only works on 2D images, not 3D.");
...
bool CTFDescription::hasPhysicalMeaning() {
	...
	if (enable_CTF) {
		...
		retval = K >= 0 && base_line >= 0 && kV >= 50 && kV <= 1000 && espr >= 0
				&& espr <= 20 && ispr >= 0 && ispr <= 20 && Cs >= 0 && Cs <= 20
				&& Ca >= 0 && Ca <= 3 && alpha >= 0 && alpha <= 5 && DeltaF >= 0
				&& DeltaF <= 1000 && DeltaR >= 0 && DeltaR <= 100 && Q0 >= 0
				&& Q0 <= 0.40 && DeltafU >= 0 && DeltafV >= 0
				&& getValueAt() >= 0;
#ifdef DEBUG
	...
	if (enable_CTFnoise) {
		...
		retval2 = base_line >= 0 && gaussian_K >= 0 && sigmaU >= 0
				&& sigmaV >= 0 && sigmaU <= 100e3 && sigmaV <= 100e3 && cU >= 0
				&& cV >= 0 && sqU >= 0 && sqV >= 0 && sqrt_K >= 0
				&& gaussian_K2 >= 0 && sigmaU2 >= 0 && sigmaV2 >= 0
				&& sigmaU2 <= 100e3 && sigmaV2 <= 100e3 && cU2 >= 0 && cV2 >= 0
				&& gaussian_angle >= 0 && gaussian_angle <= 90
				&& sqrt_angle >= 0 && sqrt_angle <= 90 && gaussian_angle2 >= 0
				&& gaussian_angle2 <= 90;
		if (min_sigma > 0)
...
void generateCTFImageWith2CTFs(const MetaData &MD1, const MetaData &MD2,
		int Xdim, MultidimArray<double> &imgOut) {
...
double errorBetween2CTFs(MetaData &MD1, MetaData &MD2, size_t Xdim,
...
#define DEBUG
	...
	std::cerr << "CTF1.Tm" << CTF1.Tm << std::endl;

	std::cerr << "MD1" << MD1 << std::endl;
	std::cerr << "MD2" << MD2 << std::endl;

	Image<double> img1(Xdim, Xdim);
	Image<double> img2(Xdim, Xdim);
	Image<double> img3(Xdim, Xdim);
	...
	FOR_ALL_ELEMENTS_IN_ARRAY2D(imgOut)
	{
		...
		CTF1.precomputeValues(XX(freq),YY(freq));
		CTF2.precomputeValues(XX(freq),YY(freq));
		error += abs(CTF1.getValueAt()  - CTF2.getValueAt() );
...
double errorMaxFreqCTFs(MetaData &MD1, MetaData &MD2) {
