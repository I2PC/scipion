...
void MpiProgAngularProjectionMatching::processAllImages()
{
    if (node->isMaster())//master distribute jobs
    {
        ...
        while (finishingWorkers < node->size - 1)
        {
            ...
            MPI_Recv(&processedImages, 1, MPI_UNSIGNED_LONG, MPI_ANY_SOURCE, TAG_JOB_REQUEST, MPI_COMM_WORLD, &status);
...
bool MpiProgAngularProjectionMatching::distributeJobs(size_t * imagesToSent, int node)
{
    int node_index = last_chunk[node];
    static bool reached_last_chunk = false;

    if (node_index == -1 || chunk_mysampling.my_exp_img_per_sampling_point[node_index].empty())
    {
...
bool MpiProgAngularProjectionMatching::requestJobs(std::vector<size_t> &imagesToProcess)
{
    ...
    MPI_Send(&numberOfImages, 1, MPI_UNSIGNED_LONG, 0, TAG_JOB_REQUEST, MPI_COMM_WORLD);
    //Get new job to do
    MPI_Recv(imagesBuffer, mpi_job_size + 1, MPI_UNSIGNED_LONG, 0, TAG_JOB_REPLY, MPI_COMM_WORLD, &status);
...
void MpiProgAngularProjectionMatching::computeChunks()
{
    ...
    while (remaining_points == 0)
    {
        ...
        if (verbose)
            std::cout << "New chunk_angular_distance " << chunk_angular_distance << std::endl;
    ...
    if (verbose)
    {
        ...
        << "biggest subset (EXPERIMENTAL images per chunk): " << max_number_of_images_in_around_a_sampling_point << std::endl
        << "maximun number of references in memory: " <<  max_nr_refs_in_memory << std::endl;
    }
