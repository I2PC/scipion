...
void MpiProgAngularProjectionMatching::processAllImages()
{
    if (node->isMaster())    //master distribute jobs
    {
        ...
        size_t processedImages = 0, finishedImages = 0, totalImages =
                                     DFexp.size();
        MPI_Status status;
        ...
        int node)
{
    ...
    size_t assigned_images =
        XMIPP_MIN(mpi_job_size, chunk_mysampling.my_exp_img_per_sampling_point[node_index].size());
        ...
        int sym_order)
{
    ...
    while (1)
    {
        ...
        double areaVoronoiRegionReferenceLibrary =
            2
            * (3
               * (acos(
                      //NEXT ONE IS SAMPLING NOT ANOTHERSAMPLING
                      cos(mysampling.sampling_rate_rad)
                      / (1
                         + cos(
                             mysampling.sampling_rate_rad))))
               - PI);
        int number_of_images_that_fit_in_a_chunck_neigh = ceil(
                    area_chunck_neigh / areaVoronoiRegionReferenceLibrary);
        //#define DEBUG
#ifdef DEBUG
