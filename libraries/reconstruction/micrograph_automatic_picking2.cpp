...
AutoParticlePicking2::AutoParticlePicking2(const FileName &fn, Micrograph *_m,
	...
	classifier2.setParameters(1.0, 0.25); //(1.0,0.25);//(8,0.125);//(1.0,1.0);//(1,0.25);//
...
void filterBankGenerator(MultidimArray<double> &inputMicrograph,
		const FileName &fnFilterBankStack, int filter_num) {
...
void AutoParticlePicking2::polarCorrelation(MultidimArray<double> &Ipolar,
	...
	for (int i = 0; i < (filter_num - corr_num); i++)
		for (int j = 1; j <= corr_num; j++) {
			correlationBetweenPolarChannels(i, i + j, nF++, Ipolar, IpolarCorr,
					aux);
		}
}
...
void AutoParticlePicking2::buildVector(MultidimArray<double> &inputVec,
		MultidimArray<double> &staticVec, MultidimArray<double> &featureVec) {
...
void AutoParticlePicking2::buildInvariant(
		MultidimArray<double> &invariantChannel, int x, int y) {
...
void AutoParticlePicking2::trainPCA(const FileName &fnPositiveFeat) {
	...
	for (int i = 1; i <= num_correlation; i++) {
		for (int j = 0; j < steps; j++) {
			positiveInvariant.readMapped(fnPositiveInvariatn,
					j * num_correlation + i);
...
void AutoParticlePicking2::trainSVM(const FileName &fnModel,
	...
	if (numClassifier == 1) {
		...
		//        classifier->saveModel(fnModel);
		std::cerr << "SVM1 Has Been Trained!!!! " << fnModel.c_str()
				<< std::endl;
	} else {
		...
		classifier2.SaveModel(fnModel);
		//delete classifier2;
		std::cerr << "SVM2 Has Been Trained!!!! " << fnModel.c_str()
				<< std::endl;
	}
}
int AutoParticlePicking2::automaticallySelectParticles(bool use2Classifier) {
	...
	int num = positionArray.size() * (5.0 / 100.0);
	for (int k = 0; k < num; k++) {
...
void AutoParticlePicking2::add2Dataset(const FileName &fn_Invariant,
		const FileName &fnParticles, int label) {
	ImageGeneric positiveInvariant;
	MultidimArray<double> vec;
	...
	for (int i = 0; i < num_correlation; i++) {
		...
		for (int j = 0; j < NPCA; j++) {
			...
			for (int k = 0; k < steps; k++) {
				positiveInvariant.readMapped(fn_Invariant,
						k * num_correlation + i + 1);
...
void AutoParticlePicking2::add2Dataset() {
	int yDataSet = YSIZE(dataSet);
	dataSet.resize(1, 1, yDataSet + auto_candidates.size(),
			num_correlation * NPCA + 12);
	classLabel.resize(1, 1, 1, YSIZE(dataSet));
	for (int n = yDataSet; n < XSIZE(classLabel); n++)
		classLabel(n) = 3;
	for (int i = 0; i < auto_candidates.size(); i++) {
...
void AutoParticlePicking2::extractPositiveInvariant(
		const FileName &fnInvariantFeat, const FileName &fnParticles) {
...
void AutoParticlePicking2::extractParticle(const int x, const int y,
		MultidimArray<double> &filter, MultidimArray<double> &particleImage,
		bool normal) {
...
void AutoParticlePicking2::extractNonParticle(
		std::vector<Particle2> &negativePosition) {
	int endX, endY;
...
void AutoParticlePicking2::buildSearchSpace(
		std::vector<Particle2> &positionArray) {
	int endX, endY;
...
void AutoParticlePicking2::applyConvolution()
{
	...
	FileName("R_Images.stk").deleteFile();
	Image<double> II;
	std::cerr<<microImage<<std::endl;
	particleAvg.printShape();
	II().alias(particleAvg);
	II.write("Average.stk", ALL_IMAGES, true, WRITE_APPEND);
...
void ProgMicrographAutomaticPicking2::run() {
	...
	AutoParticlePicking2 *autoPicking = new AutoParticlePicking2(fn_micrograph,
			&m, size, filter_num, NPCA, corr_num);

	if (mode != "train") {
		// Resize the Micrograph
		selfScaleToSizeFourier((m.Ydim) * autoPicking->scaleRate,
				(m.Xdim) * autoPicking->scaleRate, autoPicking->microImage(),
				2);
	...
	if (mode == "buildinv") {
		...
		if (fnAvgModel.exists()) {
			...
			autoPicking->particleAvg.initZeros(autoPicking->particle_size + 1,
					autoPicking->particle_size + 1);
	...
	if (mode == "train") {
		...
		autoPicking->add2Dataset(fnInvariant + "_Positive.stk",
				fnParticles + "_Positive.stk", 1);
		autoPicking->add2Dataset(fnInvariant + "_Negative.stk",
				fnParticles + "_Negative.stk", 2);
