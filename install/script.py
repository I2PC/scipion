...
def defineBinaries(args=None):
    ...
    tensorflow = env.addPipModule('tensorflow-gpu', pipCmd="pip install https://storage.googleapis.com/tensorflow/linux/gpu/tensorflow_gpu-1.4.1-cp27-none-linux_x86_64.whl", target='tensorflow*', default=False)
    cv2 = env.addPipModule('opencv-python', "3.4.2.17", target="cv2", default=False)
    env.addPipModule('Keras', '2.1.1', target='keras', default=False, deps=[cv2, tensorflow])
