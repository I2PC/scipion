...
# {hidden} Split data into random halves? 
"""
If set to yes, the experimental images will be split into two random subsets, which will be refined completely independently. The FSC between the two independent reconstructions will be used to estimate the power of the signal in the reference structure (tau2). This way, the refinement is guaranteed to be free from overfitting. Because random halves of the data will give rise to lower resolutions, upon convergence of the refinement of the random halves, one can join the entire data set in a final reconstruction. This may be done using the option below. Note that after joining the data together, one should NOT attempt to refine the structure any further, as overfitting may then sneak in again. 

NOTE: this option only works when refining a SINGLE class. If your data is heterogenous, first classify the data without the division into random halves, and then refine each class separately using this option.
"""
SplitRandomHalves = False

# {hidden} Join random halves
JoinRandomHalves = False
...
# Oversampling order
"""If a value larger than 0 is used, an adaptive approach to the angular sampling will be pursued. In that case, the probabilities will first be evaluated on a coarser angular sampling grid. Only for the coarse angular sampling points that contribute to 99.9% of the weights, probabilities for the finer sampling are considered. A value of 1 means the oversampled grid is twice as fine as the coarse grid. A value of 2 means that the oversampled grid is 4x as fine. Recommended value = 1.
"""
OversamplingOrder = 1
