...
class ProtRelionClassifier(ProtRelionBase):
    ...
    def validate(self):
        print "validate self.DoContinue", self.DoContinue
        if self.DoContinue:
            self.validateClassifyContinue()
        else:
            self.validateClassify()
    ...
    def validateClassifyContinue(self):
        ...
        errors=[]
        if '3D/RelionClass' not in self.inputFileName:
            message = 'File <%s> has not been generated by a relion classify protocol'%self.inputFileName
            if '3D/RelionRef' in self.inputFileName:
