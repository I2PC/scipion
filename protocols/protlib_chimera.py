...
class XmippChimeraClient:
    ...
    def __init__(self, volfile, port, angulardist=None,
                 voxelSize=None, extraCommandList=None):
        ...
        if '@' in volfile:
            ...
            file = volfile 
        self.extraCommandList = extraCommandList
        ...
        self.initVolumeData()#fills self.vol

        #angular dist is an special case should not be here ROB
        if angulardist:
    ...
    def openVolumeOnServer(self, volume):
        ...
        if not self.extraCommandList is None:
            for command in self.extraCommandList:
...
class XmippVirusSym(XmippChimeraClient):
    
    def __init__(self, volfile, port, voxelSize=None, h=5, k=0, sym='n25',radius=100):
        angulardist = None
        extraComamndList=['move -64,-64,-64 models #0   ',
                          'hkcage %d %d orientation %s'%(h,k,sym),
                          'hkcage %d %d orientation %s'%(h,7,sym)
        ]
        XmippChimeraClient.__init__(self, volfile, port,
                                    angulardist, voxelSize,extraComamndList)
