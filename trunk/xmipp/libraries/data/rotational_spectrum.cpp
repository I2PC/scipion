    ...
    double xr1, double xr2, double xdr, double xr)
{
    ...
    *rp1, *rp2, *sp, *c, *s;
    ...
    if ((c = (double *) calloc (5191, sizeof (double))) == NULL ||
        ...
        REPORT_ERROR(1,"compute_rotational_spectrum::no memory");
    ...
    if (ncol < 0)
        ...
        ncol=1+ncol/ndr;
    if (ncol == 0)
        REPORT_ERROR(1,"compute_rotational_spectrum::Incorrect data");
...
{
    rl=AtoI(get_param(argc,argv,"-r1"));
    rh=AtoI(get_param(argc,argv,"-r2"));
    dr=AtoI(get_param(argc,argv,"-rInc","1"));
    x0=AtoF(get_param(argc,argv,"-x0","-1"));
    y0=AtoF(get_param(argc,argv,"-y0","-1"));
    numin=AtoI(get_param(argc,argv,"-low","1"));
    numax=AtoI(get_param(argc,argv,"-high","15"));
}
