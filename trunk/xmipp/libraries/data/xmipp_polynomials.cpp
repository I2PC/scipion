...
#define ZERNIKE_ORDER(n) ceil((-3+sqrt(9+8*(float)(n)))/2.);
...
void PolyZernikes::create(const Matrix1D<int> & coef)
{
    ...
    for (int nZ = 0; nZ < VEC_XSIZE(coef); ++nZ)
    {
        ...
        else
        {
            ...
            std::cout << "Matrix " << nZ << " p q :" << p << ", " << q << " " << std::endl;
        ...
        std::cout << "Matrix " << nZ << std::endl << *fMatT << std::endl;
...
{
    //First we generate the representation of the Zernike polynomials
    this->create(coef);
    ...
    int iMaxDim2 = 2./std::max(xdim,ydim);

    int index = 0;
    //Actually polOrder corresponds to the polynomial order +1
    int polOrder=ZERNIKE_ORDER(numZer);
    PR(polOrder);
    im.setXmippOrigin();
    ...
    FOR_ALL_ELEMENTS_IN_ARRAY2D(im)
    {
        ...
        for (int px = 0; px <= polOrder; ++px)
        {
            ...
            for (int py = 0; py <= polOrder; ++py)
                dMij(polValue,px,py) = xpx*std::pow(y,py);
        }

        int idxZerMat=0;
        for (int n=0; n < VEC_XSIZE(coef); ++n)
        {
        	Matrix2D<int> *fMat = &fMatV[n];
            if (fMat == NULL)
                continue;
    ...
    DIRECT_A2D_ELEM(save(),i,j)=dMij(zerMat,i*XSIZE(im)+j,1);
    save.write("PPP0.xmp");

   //Matrix1D<double> oneDimIm  =  Matrix1D<double>(xdim*ydim);
   // Matrix1D<double> coefZer =  Matrix1D<double>(numZer);

    //oneDimIm.vdata = (double*)im.data;
    //coefZer = zerMat.inv(zerMat)*oneDimIm;
