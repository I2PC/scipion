...
void range_adjust(T minF, T maxF)
{
    ...
    double slope;
    if (max0!=min0)
        slope=(double)(maxF-minF)/(double)(max0-min0);
    else
        slope=0;
...
void statistics_adjust(double avgF, double stddevF)
{
    ...
    if (stddev0!=0)
        a=(double)stddevF/(double)stddev0;
    else
        a=0;
...
{
    if (MULTIDIM_SIZE(*this)!=MULTIDIM_SIZE(op2))
        return false;
    FOR_ALL_ELEMENTS_IN_MULTIDIM_ARRAY(*this)
    if (ABS(mi-MULTIDIM_ELEM(op2,i))>accuracy)
        return false;
    return true;
}
