...
void regionGrowing2D(const MultidimArray<double> &I_in,
	...
	/* Fill the seed coordinates */A2D_ELEM(I_out, i, j) = filling_colour;
...
void regionGrowing3D(const MultidimArray<double> &V_in,
	...
	/* Fill the seed coordinates */A3D_ELEM(V_out, k, i, j) = filling_colour;
...
void removeSmallComponents(MultidimArray<double> &I, int size,
	...
	FOR_ALL_ELEMENTS_IN_ARRAY2D(label)
		nlabel((int) (label(i, j)))++;FOR_ALL_ELEMENTS_IN_ARRAY2D
	(label)
		...
		Matrix2D<double> &R, CorrelationAux &aux)
{
	...
	VolumeAlignmentAux aux2;
	if (eulerAngles[0]=='Z')
		bestAngle1 = fastBestRotationAroundZ(IrefCylZ, I, aux, aux2);
	else if (eulerAngles[0]=='Y')
		bestAngle1 = fastBestRotationAroundY(IrefCylY, I, aux, aux2);
	else
...
/** Read from program command line */
void MedianFilter::readParams(XmippProgram * program) { //Do nothing by now
}
