...
public class Sphere {
    ...
    private final static String TEMPDIR_PATH = System.getProperty("java.io.tmpdir") + "/viewer3D";
    ...
    private SphereCell getTableCell(Point2d point) {
        return sphereMap[(int) point.x][(int) point.y];
    }
    ...
    public Vector<String> getFiles(double rot, double tilt) {
        ...
        // File names are stored at startup so every cell contains a list of
        // the ones affecting them.
        return getTableCell(point).fileNames;
    }
    ...
    public String analyzeProjection(MultidimArrayd xmippVolume, double rot, double tilt, int w, int h) {
        ...
        try {
            ...
            // * Builds scoreFileName so aligned images can be shown at table.
            scoreFile = path + "xmipp_sel_file_score.txt";
    ...
    private static void xmipp_classify_analyze_cluster(String selfile, String reffile, String oext) {
        try {
            //xmipp_classify_analyze_cluster -i file.sel -ref 12205_000001.xmp -produceAligned -oext .ali
            Process p = Runtime.getRuntime().exec(
                    ...
                    + " -odir " + System.getProperty("java.io.tmpdir") + "/viewer3D/");
