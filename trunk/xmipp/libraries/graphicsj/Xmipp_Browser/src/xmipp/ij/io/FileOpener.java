 ...
 *       new FileOpener(fi).openThumbnail();
 *     }
 *   }
 * </pre>
 */
public class FileOpener {
    ...
    /**
     * @author Juanjo Vega
     * Same as openThumbnail, with scaling.
     */
    public ImagePlus openThumbnail(boolean show, int slice, int w, int h) {
        ImagePlus imp = null;
        Object pixels;
        ProgressBar pb = null;
        ImageProcessor ip;
        ...
        ColorModel cm = createColorModel(fi);
        if (fi.nImages > 1) {
            return openStackThumbnail(cm, show, slice, w, h);
        }
        switch (fi.fileType) {
            case FileInfo.GRAY8:
            case FileInfo.COLOR8:
            ...
            case FileInfo.GRAY24_UNSIGNED:
            case FileInfo.GRAY64_FLOAT:
                pixels = readPixelsThumbnail(fi, fi.offset, w, h);
                if (pixels == null) {
                    return null;
                }
                width = w;
                ...
                //System.out.println(" >> width * height:" + (width * height) + " -> " + ((float[]) pixels).length);
     ...
     */
    /** Opens a stack of images. */
    ImagePlus openStackThumbnail(ColorModel cm, boolean show, int slice, int w, int h) {
        ImageStack stack = new ImageStack(w, h, cm);
        ...
        try {
            ...
            // Calculates offset for required slice.
            skip += (slice - 1) * (fi.gapBetweenImages + (fi.getBytesPerPixel() * fi.width * fi.height));
            pixels = reader.readPixelsThumbnail(is, skip, w, h);
            stack.addSlice(null, pixels);
     ...
     */
    Object readPixelsThumbnail(FileInfo fi, long skip, int w, int h) {
        Object pixels = null;
        try {
            InputStream is = createInputStream(fi);
            if (is == null) {
            ...
            ImageReader reader = new ImageReader(fi);
//            IJ.write(fi.fileType + " > " + FileInfo.GRAY32_FLOAT + " / N_images: " + fi.nImages);
