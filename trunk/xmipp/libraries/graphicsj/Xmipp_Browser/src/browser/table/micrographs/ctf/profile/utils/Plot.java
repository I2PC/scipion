...
public class Plot {
    ...
    public void setInvert(boolean invert) {
        ...
        if (invert && xValuesInversed == null) {
            xValuesInversed = invert(xValues);
        }
    }
    ...
    public float[] getXValues() {
        float xs[] = invert ? xValuesInversed : xValues;

        double[] a = Tools.getMinMax(xs);
        xMin = invert ? a[1] : a[0];
        xMax = invert ? a[0] : a[1];
    ...
    void drawTicksEtc() {
        ...
        if ((flags & (X_NUMBERS + X_TICKS + X_GRID)) != 0) {
            ...
            if (digits > 5) {
                digits = -3; // use scientific notation
            }
            int y1 = TOP_MARGIN;
            ...
            for (int i = 0; i <= (i2 - i1); i++) {
                ...
                if ((flags & X_NUMBERS) != 0) {
                    if (invert) {   // Shows inversed range?
                        v = v == 0 ? 0 : 1 / v;
                    }
