...
public class JFrameVolumeTable extends JFrame implements iNormalizeListener {
    ...
    public void addVolume(String dir, String file, int nslices) {
        jPanelTable.addSelVolume(dir, file);

        setTitle(LABELS.TITLE_TABLE_WINDOW_SELFILE(file, nslices));
    }

    public void addVolumeFile(File file) {
        int nslices = jPanelTable.addVolumeFile(file);

        setTitle(LABELS.TITLE_TABLE_WINDOW_SELFILE(file.getName(), nslices));
    }

    public void addSelFile(String file) {
        ...
        jPanelTable.addSelVolume(parent, name);

        setTitle(parent + File.pathSeparator + name);
    }
    ...
    private void autoAdjustColumns() {
        if (autoAdjustColumns) {
            int displayableColumns = jPanelTable.getDisplayableColumns();

            if (jPanelTable.getColumnsCount() != displayableColumns) {
    ...
    private void jsGoToImageStateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_jsGoToImageStateChanged
        ...
        if (value < 0) {
            value = 0;
        } else if (value > jPanelTable.getItemsCount() - 1) {
            value = jPanelTable.getItemsCount() - 1;
        }
