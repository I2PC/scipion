...
public class PlotWindow extends ImageWindow implements ActionListener, ClipboardOwner {
    ...
    void showList() {
        ...
        if (plot.errorBars != null) {
            ...
            for (int i = 0; i < plot.nPoints; i++) {
                if (saveXValues) {
                    sb.append(IJ.d2s(xValues[i], xdigits) + "\t" + IJ.d2s(plot.yValues[i], ydigits) + "\t" + IJ.d2s(plot.errorBars[i], ydigits) + "\n");
                } else {
            ...
            for (int i = 0; i < plot.nPoints; i++) {
                if (saveXValues) {
                    sb.append(IJ.d2s(xValues[i], xdigits) + "\t" + IJ.d2s(plot.yValues[i], ydigits) + "\n");
                } else {
    ...
    void saveAsText() {
        ...
        for (int i = 0; i < plot.nPoints; i++) {
            if (saveXValues) {
                pw.println(IJ.d2s(xValues[i], xdigits) + "\t" + IJ.d2s(plot.yValues[i], ydigits));
            } else {
    ...
    void copyToClipboard() {
        ...
        for (int i = 0; i < plot.nPoints; i++) {
            if (saveXValues) {
                pw.print(IJ.d2s(xValues[i], xdigits) + "\t" + IJ.d2s(plot.yValues[i], ydigits) + "\n");
            } else {
