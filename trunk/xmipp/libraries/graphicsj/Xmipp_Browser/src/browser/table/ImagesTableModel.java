...
public class ImagesTableModel extends AbstractTableModel {
    ...
    System.out.println(i + ": " + selectedItems.elementAt(i).getLabel());
    }
    System.out.println(" > Focused: " + (focusedItem != null ? focusedItem.getLabel() : null));
    System.out.println("** ** ** ** ** **");
    }
     */
    ...
    /*
    private void print(Vector<TableImageItem> vector) {
    for (int i = 0; i < vector.size(); i++) {
    TableImageItem tableImageItem = vector.elementAt(i);
    System.out.println(tableImageItem.getLabel());
    }
    }
     */
    ...
    public void addImage(ImageItem itemImage) {
        data.add(new TableImageItem(itemImage.getFile(), cache));
        /*
    ...
    public void addVolume(FileImageItem itemImage) {
        for (int i = 0; i < itemImage.nslices; i++) {
            data.add(new TableImageItem(itemImage.getFile(), cache, i + 1));
            /*
            if (lastSize == LAST_SIZE.ROWS) { // Last time it was resized by rows.
            cols = getNecessaryCols(rows);
            } else {     // Last time it was resized by columns.
    ...
    public void addVolume(SelFileItem itemImage) {
        ...
        for (int i = 0; i < fileNames.length; i++) {
            data.add(new TableImageItem(new File(fileNames[i]), cache));
            /*
            if (lastSize == LAST_SIZE.ROWS) { // Last time it was resized by rows.
            cols = getNecessaryCols(rows);
            } else {     // Last time it was resized by columns.
