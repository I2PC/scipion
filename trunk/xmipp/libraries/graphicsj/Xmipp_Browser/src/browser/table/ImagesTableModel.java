...
//        updateNormalizeValues();
    ...
    public void toggleSelected(int row, int col) {
        ...
        if (item != null) {
            setSelected(row, col, !item.selected);
        }
    }
    ...
    public int addVolumeFile(File file) {
        ...
        for (int i = 0; i < ip.getStackSize(); i++) {
            data.add(new TableImageItem(file, cache, i, this));
        }
    ...
    public void setRows(int rows) {
        if (rows > 0 && rows <= getSize()) {
            ...
            System.out.println("rows: " + rows + " >>> Necessary cols: " + cols);
    ...
    public void setColumns(int cols) {
        if (cols > 0 && cols <= getSize()) {
            ...
            System.out.println("cols: " + cols + " >>> Necessary rows: " + rows);
