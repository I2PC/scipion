...
public class XmippMenuBar extends DynamicMenuBar implements ActionListener {
    ...
    // *** *** ***
    private Menu menuMisc = new Menu(LABELS.OPERATION_MENU_PLUGINS);
    private MenuItem itemTemplateMatching = new MenuItem(LABELS.OPERATION_TEMPLATE_MATCHING);
    private MenuItem itemFlowJ = new MenuItem(LABELS.OPERATION_FLOWJ);
    private MenuItem itemFlow3J = new MenuItem(LABELS.OPERATION_FLOW3J);
    private MenuItem itemSurfaceJ = new MenuItem(LABELS.OPERATION_SURFACEJ);
    private MenuItem itemObjectJ = new MenuItem(LABELS.OPERATION_OBJECTJ);
    ...
    public XmippMenuBar(Container parent, ImagePlus imp, boolean canPoll) {
        ...
        itemUntiltStack.setEnabled(imp.getStackSize() > 1);
        itemPCA.setEnabled(imp.getStackSize() > 1);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            if (e.getSource() == itemFlipV) {
                ...
                imp2.setProcessor(imp.getTitle() + "-duplicated", imp.getProcessor().duplicate());

                ImagesWindowFactory.openImageWindow(imp2, false);
