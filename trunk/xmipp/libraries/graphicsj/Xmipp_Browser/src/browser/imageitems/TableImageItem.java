...
public class TableImageItem {
    ...
    public String getKey() {
        return item.getKey() + (slice != FileOpener.MID_SLICE ? "-" + slice : "");
    }

    public String getLabel() {
        return item.getLabel() + (slice != FileOpener.MID_SLICE ? "[" + slice + "]" : "");
    }
...
//        this.max = max;
    ...
    public ImagePlus getPreview(int w, int h) {
        ...
        // If not in cache...
        if (preview == null) {
            preview = item.loadPreview(w, h, slice);   // ...loads it from disk.
