...
public class HandleExtraFileTypes extends ImagePlus implements PlugIn {
    ...
    private Object tryOpen(String directory, String name, String path) {
        ...
        // COSS: read Spider files
        if (name.endsWith(".img") || name.endsWith(".xmp") || name.endsWith(".vol") || name.endsWith(".spi")) {
            return tryPlugIn("Spider_Reader", path);
        }

        // Juanjo Vega:
        // TIA
        if (name.endsWith(".ser")) {
            return tryPlugIn("TIA_Reader", path);
        }
