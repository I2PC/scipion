...
public class ImageReader {
     ...
     * @author Juanjo Vega
     *
     * Similar to read16bitImageThumbnail, but using scaling.
     *
     * @param in
     * @param scaleX
     * @param scaleY
     ...
     * @throws IOException
     */
    short[] read16bitImageThumbnail(InputStream in, int w, int h) throws IOException {
        double scaleX = (double) fi.width / (double) w;
        double scaleY = (double) fi.height / (double) h;

        int elementsPerRow = fi.width;
        ...
        for (int j = 0; j < h; j++) {
            for (int i = 0; i < w; i++) {
                int nextX = (int) Math.round(scaleX * i);
                int nextY = (int) Math.round(scaleY * j);

                if (currentY == nextY) {// Already at right row.
                    if (currentX < nextX) {
                        skip = nextX - currentX - 1; // Skip horizontally.

                        currentX = nextX; // Refresh currentX.
                    }
                } else {
                    // Skip remaining items at current row.
                    skip = elementsPerRow - (currentX + 1);

                    // Skips required cols.
                    skip += (nextY - currentY - 1) * elementsPerRow;

                    // Skip horizontally to nextX
                    skip += nextX;

                    // New row.
                    currentX = nextX; // Refresh currentX.
                    currentY = nextY; // Refresh currentY.
                }

                in.skip(skip * bytesPerPixel);

                // ...before reading an item.
                in.read(buffer, 0, bytesPerPixel);

                // Converts byte according to big endian or little endian.
                if (fi.intelByteOrder) {
                    tmp = (short) ((((buffer[1] & 0xff) << 8) | (buffer[0] & 0xff)) + 32768);
                } else {
                    tmp = (short) (((buffer[0] & 0xff) << 8) | (buffer[1] & 0xff));
                }

                // Convert and assign bytes.
                if (fi.fileType == FileInfo.GRAY16_SIGNED) {
                    pixels[j * w + i] = tmp;
                } else {
                    pixels[j * w + i] = tmp;
     ...
     * @author Juanjo Vega
     *
     * Similar to read32bitImageThumbnail, but using scaling.
     *
     * @param in
     * @param scaleX
     * @param scaleY
     ...
     * @throws IOException
     */
    float[] read32bitImageThumbnail(InputStream in, int w, int h) throws IOException {
        double scaleX = (double) fi.width / (double) w;
        double scaleY = (double) fi.height / (double) h;
        ...
        for (int j = 0; j < h; j++) {
            for (int i = 0; i < w; i++) {
                ...
                // Converts byte according to big endian or little endian.
                if (fi.intelByteOrder) {
                    tmp = (int) (((buffer[3] & 0xff) << 24) | ((buffer[2] & 0xff) << 16) | ((buffer[1] & 0xff) << 8) | (buffer[0] & 0xff));
                } else {
                    tmp = (int) (((buffer[0] & 0xff) << 24) | ((buffer[1] & 0xff) << 16) | ((buffer[2] & 0xff) << 8) | (buffer[3] & 0xff));
                }

                // Convert and assign bytes.
                if (fi.fileType == FileInfo.GRAY32_FLOAT) {
                    pixels[j * w + i] = Float.intBitsToFloat(tmp);
                } else if (fi.fileType == FileInfo.GRAY32_UNSIGNED) {
                    pixels[j * w + i] = (float) (tmp & 0xffffffffL);
                } else {
                    pixels[j * w + i] = tmp;
                }
            }
    ...
    float[] read32bitImageThumbnail_TMP(InputStream in, double scaleX, double scaleY) throws IOException {
        double skipX = Math.ceil(1 / scaleX - 1);   // # of pixels to skip horizontally.
        double skipY = Math.ceil(1 / scaleY - 1);   // # of pixels to skip vertically.
        int w = (int) (width * scaleX); // New width
        int h = (int) (height * scaleY); // New height
        float[] pixels = new float[w * h];
        byte[] buffer = new byte[bytesPerPixel];

        IJ.write("Skipping, X: " + skipX + " / Y: " + skipY);

        double bytesPerRow = width * bytesPerPixel;
        int count;
        int tmp;
        double processedX;
        ...
        for (int j = 0; j < h; j++) {
            processedX = 0;
            for (int i = 0; i < w; i++) {
                count = in.read(buffer, 0, bytesPerPixel);
                ...
                // Convert and assign bytes.
                if (fi.fileType == FileInfo.GRAY32_FLOAT) {
                    pixels[index] = Float.intBitsToFloat(tmp);
                } else if (fi.fileType == FileInfo.GRAY32_UNSIGNED) {
                    pixels[index] = (float) (tmp & 0xffffffffL);
                } else {
                    pixels[index] = tmp;
                }
                ...
                in.skip((long) (skipX * bytesPerPixel));

                processedX += (skipX * bytesPerPixel) + count;
            }
            //System.out.println("Processed " + processedX / bytesPerPixel + " pixels / " + bytesPerRow / bytesPerPixel);

            // Skips remaining pixels for row.
            in.skip((long) (bytesPerRow - processedX));

            // Skips complete rows.
            in.skip((long) (skipY * bytesPerRow));
        }
     ...
     * @author Juanjo Vega
     *
     * Similar to readPixelsThumbnail, with scaling.
     *
     * @param in
     * @param skipCount
     ...
     * @return
     */
    public Object readPixelsThumbnail(InputStream in, long skipCount, int w, int h) {
        this.skipCount = skipCount;
        showProgressBar = false;
        Object pixels = readPixelsThumbnail(in, w, h);
        if (eofErrorCount > 0) {
            return null;
        } else {
            return pixels;
     ...
     * @return
     */
    public Object readPixelsThumbnail(InputStream in, int h, int w) {
        Object pixels;
        startTime = System.currentTimeMillis();
        try {
            switch (fi.fileType) {
                ...
                case FileInfo.GRAY16_UNSIGNED:
                    bytesPerPixel = 2;
                    skip(in);
                    pixels = (Object) read16bitImageThumbnail(in, w, h);
                    break;
                case FileInfo.GRAY32_INT:
                case FileInfo.GRAY32_UNSIGNED:
                case FileInfo.GRAY32_FLOAT:
                    bytesPerPixel = 4;
                    skip(in);
                    pixels = (Object) read32bitImageThumbnail(in, w, h);
                    break;
                case FileInfo.GRAY64_FLOAT:
                    bytesPerPixel = 8;
                    skip(in);
