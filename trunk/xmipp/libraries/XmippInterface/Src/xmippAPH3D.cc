...
//   fh_aph.peek();
   getline(fh_aph,line);
   try {
       getline(fh_aph,line);Reduce_phase_accuracy = AtoF(line.c_str()+47);
       getline(fh_aph,line);a = AtoF(line.c_str()+47);
       getline(fh_aph,line);b = AtoF(line.c_str()+47);
       getline(fh_aph,line);gamma = AtoF(line.c_str()+47);
       getline(fh_aph,line);c = AtoF(line.c_str()+47);
       getline(fh_aph,line);resolution = AtoF(line.c_str()+47);
       getline(fh_aph,line);Amp_Scale = AtoF(line.c_str()+47);
       } catch (...) {
...
if(Space_Group==0)
   ...
   FOM.init_zeros(lmax-lmin+1,kmax-kmin+1,hmax-hmin+1);
      STARTINGX(FOM)=hmin; STARTINGY(FOM)=kmin;
      STARTINGZ(FOM)=lmin;
