    ...
    void FillauxGridVolumewithClicks(void)
    {
        try
        {
            FOR_ALL_ELEMENTS_IN_MATRIX3D(VOLMATRIX(*_FCC_Click))
            {
                ...
                {
                    cout << k << " " << i << " " << j << "\t\t" ;
                    printb(cout, VOLVOXEL((*_FCC_Click), k, i, j));
                    cout << endl;
...
//    Copy_to_Gridvolume_debug_and_Remove_Mask();
    ...
    /** Get Total Energy */
    long int Get_Total_energy(void)
    {
        return (Total_Energy);
    }
    /** Update Total Energy */
    void Update_Total_energy(int energy_change)
    {
        Total_Energy += energy_change;
    }  /** Remove gridvolume mask */
