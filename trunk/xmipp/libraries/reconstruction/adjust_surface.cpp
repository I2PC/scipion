...
void Prog_Adjust_Surface_Parameters::read(int argc, char **argv) {
   ...
   } else {angle0=angleF=0; angle_step=1;}
   ...
   } else {shiftX0=shiftY0=shift_dist=0; shift_step=1;}
   ...
   } else {scaleX0=scaleXF=1; scaleX_step=1;}
   ...
   } else {scaleY0=scaleYF=1; scaleY_step=1;}
...
void Prog_Adjust_Surface_Parameters::produce_Side_Info() {
   ...
   if (!given_zbottom) {zbottom0 = STARTINGZ(V()); zbottomF = FINISHINGZ(V());}
...
double eval_surface(double *p) {
   ...
   if (p[6]!=0 || p[7]!=0) {
      matrix2D<double> scale_m; scale_m.init_identity(3);
      matrix2D<double> aux;
      scale_m(0,0)=p[6];
      scale_m(1,1)=p[7];
		  ...
		  for (prm.p(4)=prm.zbottom0; prm.p(4)<=prm.zbottomF; prm.p(4)+=prm.zbottom_step)
	             for (prm.p(5)=prm.scaleX0; prm.p(5)<=prm.scaleXF; prm.p(5)+=prm.scaleX_step)
	                for (prm.p(6)=prm.scaleY0; prm.p(6)<=prm.scaleYF; prm.p(6)+=prm.scaleY_step) {
        		       ...
        		       cout << "shiftx ="; cin >> prm.p(0);
        		       cout << "shifty ="; cin >> prm.p(1);
        		       cout << "angle ="; cin >> prm.p(2);
        		       cout << "ktop (" << prm.ztop0 << "..." << prm.ztopF << ")=";
        		       cin >> prm.p(3);
        		       cout << "kbottom (" << prm.zbottom0 << "..." << prm.zbottomF << ")=";
        		       cin >> prm.p(4);
        		       cout << "scaleX ="; cin >> prm.p(5);
        		       cout << "scaleY ="; cin >> prm.p(6);
        	             }
			        ...
			        best_ang=prm.p(2);
			        best_shiftx=prm.p(0); best_shifty=prm.p(1);
                                best_scaleX=prm.p(5); best_scaleY=prm.p(6);
			        cout << "********    ";
        	             }
