...
// ==========================================================================
/* Learn particles --------------------------------------------------------- */
void AutoParticlePicking::learnParticles(int _ellipse_radius) {
...
void AutoParticlePicking::getAutoFalsePositives() {
	...
	for (int i = 0; i < imax; i++)
		if (__selection_model.addParticleTraining(__auto_candidates[i], 2))
			Nrejected++;
...
void AutoParticlePicking::getAutoTruePositives() {
	...
	int imax = __auto_candidates.size();
	for (int i = 0; i < imax; i++)
		if (__selection_model.addParticleTraining(__auto_candidates[i], 0))
			Naccepted++;
...
void AutoParticlePicking::buildPositiveVectors() {
	...
	__selection_model.addMicrographItem();
	int width, height;
	...
	if (features.size() == 3) {
		...
		if (Nalive > 0) {
			...
			for (int i = 0; i < imax; i++)
				if (__auto_candidates[i].status == 1) {
					double p;
					int votes = selection_model2.isParticle(
		...
		if (Nalive > 0) {
			...
			for (int i = 0; i < YSIZE(features_1); i++) {
				memcpy(&VEC_ELEM(trialFeatures,0), &A2D_ELEM(features_1,i,0),
						rowLength);
				int votes = selection_model3.isParticle(trialFeatures, cost);
				if (votes < 8)
					toKeep.push_back(i);
			}
...
void AutoParticlePicking::find_neighbour(const MultidimArray<double> &piece,
		...
		MultidimArray<char> &_visited, std::vector<Matrix1D<int> > &_nbr) {
...
void AutoParticlePicking::loadModels(const FileName &fn) {
	__modelRootName = fn;
	...
	__mask.fn_mask = __modelRootName + "_mask.xmp";
	__mask.generate_mask();
	__mask.get_binary_mask().setXmippOrigin();
...
/* Save models ------------------------------------------------------------- */
void AutoParticlePicking::saveModels(const FileName &_fn_root) const {
...
/* Save particles ---------------------------------------------------------- */
void AutoParticlePicking::saveAutoParticles() const {
	MetaData MD;
	...
	MD.write(__outputRoot + "." + __modelRootName.removeDirectories() + ".pos");
}
...
void ProgMicrographAutomaticPicking::defineParams() {
	...
	addParamsLine(
			"                    train <posfile>  : posfile contains the coordinates of manually picked particles");
	addParamsLine(
			"                                     : <rootname>_auto_feature_vectors.xmd contains the particle structure created by this program when used in automatic selection mode");
	addParamsLine(
	...
	addParamsLine(
			"  [--writeFeatures]             : Write the feature vectors of the automatically selected particles");
	addParamsLine(
			"                                : under the name  <rootname>_auto_feature_vectors.xmd");
	addExampleLine("Training:", false);
	addExampleLine(
			"xmipp_micrograph_automatic_picking -i micrograph.tif --particleSize 100 --mode train manual.pos --model model --thr 4 --output_root micrograph");
	addExampleLine("Automatically select particles during training:", false);
	addExampleLine(
			"xmipp_micrograph_automatic_picking -i micrograph.tif --particleSize 100 --mode autoselect --model model --thr 4  --output_root micrograph --write_features");
	addExampleLine("Automatically select particles after training:", false);
	addExampleLine(
			"xmipp_micrograph_automatic_picking -i micrograph.tif --particleSize 100 --mode autoselect --model model --thr 4  --output_root micrograph");
}
...
void ProgMicrographAutomaticPicking::run() {
	...
	if (writeFeatures)
		autoPicking->saveAutoFeatureVectors(
				fn_root + "_auto_feature_vectors.xmd");
}
