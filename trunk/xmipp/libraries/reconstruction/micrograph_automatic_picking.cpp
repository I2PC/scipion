...
void AutoParticlePicking::getAutoFalsePositives(Classification_model &_training_model)
{
...
void AutoParticlePicking::buildPositiveVectors(std::vector<int> &_idx,
...
void AutoParticlePicking::buildNegativeVectors(Classification_model &_model,
...
int AutoParticlePicking::reject_within_distance(
...
/* produceFeatures --------------------------------------------------------- */
void AutoParticlePicking::getFeatures(
...
void AutoParticlePicking::getClassesProbabilities(
...
Classification_model::Classification_model(int _classNo, int _maxTrainingVectors)
{
    __maxTrainingVectors=_maxTrainingVectors;
    init(_classNo);
}

void Classification_model::init(int _classNo)
{
    __classNo = _classNo;
...
// ==========================================================================
/* Correct particles ------------------------------------------------------- */
void AutoParticlePicking::move_particle(int _idx)
{
...
/* Delete particles ---------------------------------------------------------*/
void AutoParticlePicking::delete_particle(int _idx)
{
    if (__autoselection_done)
    {
...
void ProgMicrographAutomaticPicking::show()
{
    ...
    << "Mode         : " << mode          << std::endl
    << "Training file: " << fn_train      << std::endl
...
void ProgMicrographAutomaticPicking::defineParams()
{
    ...
    addExampleLine("xmipp_micrograph_automatic_picking -i micrograph.tif --particleSize 100 --mode train manual.pos --model model --thr 4 --output_root micrograph");
    addExampleLine("Automatically select particles during training:",false);
    addExampleLine("xmipp_micrograph_automatic_picking -i micrograph.tif --particleSize 100 --mode autoselect --model model --thr 4  --output_root micrograph --write_features");
    addExampleLine("Automatically select particles after training:",false);
    addExampleLine("xmipp_micrograph_automatic_picking -i micrograph.tif --particleSize 100 --mode autoselect --model model --thr 4  --output_root micrograph");
}
...
void ProgMicrographAutomaticPicking::run()
{
    ...
    else
    {
        ...
        if (exists(fnFalsePositives))
        {
            ...
            if (MD.size()>0)
            {
                autoPicking->loadAutoFeatureVectors(fn_root+"_auto_feature_vectors.xmd");
                int idx;
                FOR_ALL_OBJECTS_IN_METADATA(MD)
                {
