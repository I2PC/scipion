...
#define FN_ITER_BASE(iter) formatString("%s_iter%0FILENAMENUMBERLENTGHd", fn_root.c_str(), (iter))
#define FN_REFMD(base) ((base) + "_refs.xmd")
#define FN_IMGMD(base) ((base) + "_imgs.xmd")
#define FN_LOGMD(base) ((base) + "_logs.xmd")
#define FN_REF(base, refno) formatString("%0FILENAMENUMBERLENTGHd@%s_refs.stk", (refno), (base).c_str())
#define FN_VSIG(base, ifocus, ext) ((nr_focus > 1) ? formatString("%s_ctf%0FILENAMENUMBERLENTGHd%s", (base).c_str(), ((ifocus) + 1), (ext)) : ((base) + "_ctf" + (ext)))
