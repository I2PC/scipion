...
void Prog_Break_Sym_prm::process_one_image(ImageXmipp &img, int &opt_vol,
   ...
   if (verb>1) {Itmp()=projmask(); Itmp.write("mask.xmp");}
   ...
   for (int volno=0; volno<Nvols; volno++) {
     ...
     if (verb>1) {
       Itmp()=proj(); Itmp.write("proj.xmp");
       Itmp()=img(); Itmp.write("img.xmp");
     }
     for (int isym=0; isym<SL.SymsNo(); isym++) {
       ...
       proj().set_Xmipp_origin();
       if (verb>1) { Itmp()=proj(); fnt.compose("sym",isym,"xmp"); Itmp.write(fnt);}
   ...
   img.write(img.name());
   if (verb>1) { cout <<endl; cout << opt_rot<<" "<<opt_tilt<<" "<<opt_psi<<endl;}
}
