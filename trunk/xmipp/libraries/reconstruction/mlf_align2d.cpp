        ...
        int iter)
{
    ...
    if (highres_limit > 0.) int_highres_limit = ROUND(sampling * dim / highres_limit);
    else int_highres_limit = hdim;
    if (ini_highres_limit>  0.) int_ini_highres_limit = ROUND(sampling * dim / ini_highres_limit);
