 ...
 * Authors:
 *
 * Javier Rodríguez Falces (jrodriguez@cnb.uam.es)
...
void Prog_create_surface::read_input_file() {
   ...
   while (fgets (line, 200,fh_param ) != NULL) {
      ...
      switch(lineNo) {
     case 0:
            a.X()=AtoF(first_token(line),3007,
               "Prog_Project_Parameters::read: Error in lattice vector a");
            a.Y()=AtoF(next_token(),3007,
               "Prog_Project_Parameters::read: Error in lattice vector a");
            lineNo=1;
	    break;
     case 1:	
...
class Func1: public doubleFunction{ 			//actual function to be integrated
...
class Func2: public doubleFunction{ 		//actual function to be integrated
public: 									//This should be in testinteg
	double x;
	double Aperture;
	virtual double operator()() 			//overloads pure virtual
