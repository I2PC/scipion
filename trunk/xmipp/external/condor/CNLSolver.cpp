             ...
             ObjectiveFunction *of, int nnode)
{
    ...
    cacheMultInd.get(dim,1);
    cacheMultInd.get(dim,2);
...
//              BUT is more precise (no rounding error)
                ...
                if ((reduction<noise)&&(!forceTRStep))
                {
                    evalNeeded=true;
                    break;
                }
