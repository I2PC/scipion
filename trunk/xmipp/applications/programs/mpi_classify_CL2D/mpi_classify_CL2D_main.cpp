...
void CL2DClass::fitBasic(MultidimArray<double> &I, CL2DAssignment &result,
	...
	for (int i = 0; i < 3; i++) {
		...
		bestRot
				= best_rotation(polarFourierP, polarFourierI, local_transformer);
	...
	if (candidateRS.shiftx * candidateRS.shiftx + candidateRS.shifty
			* candidateRS.shifty > prm->maxShift2)
		candidateRS.corr = 0;
	else
		candidateRS.corr = corrRS;
	if (candidateSR.shiftx * candidateSR.shiftx + candidateSR.shifty
			* candidateSR.shifty > prm->maxShift2)
		candidateSR.corr = 0;
	else
