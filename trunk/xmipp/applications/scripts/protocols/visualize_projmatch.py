...
class visualize_projmatch_class:
    ...
    def __init__(self,
                 ...
                 SelectIterations,
                 VisualizeprojmatchReferences,
                 VisualizeprojmatchAvgs,
        ...
        self.VisualizeprojmatchReferences=VisualizeprojmatchReferences
        self.VisualizeprojmatchAvgs=VisualizeprojmatchAvgs
        ...
        os.chdir(protocol_projmatch.WorkingDir)

#        self.visualize_preparation()

#        self.visualize_seed_generation()

#        self.visualize_projmatch_iterations()
        
#        self.visualize_projmatch_overall()
...
#        visualization.visualize_images(self.ShowSelfiles,True,MatrixWidth)
    ...
    def visualize_seed_generation(self):
        for i in range(protocol_projmatch.NumberOfReferences):
    ...
    def visualize_projmatch_iterations(self):
        import os,glob
        iters=self.SelectIterations.split(',')
        for iter in iters:
            if (self.VisualizeprojmatchAvgs):
                selfiles=glob.glob('Runprojmatch/projmatch_it'+str(iter).zfill(5)+'_vol?????.sel')
                for selfile in selfiles:
                    self.ShowSelfiles.append(selfile)

            if (self.VisualizeprojmatchReferences):
                volumes=glob.glob('Runprojmatch/projmatch_it'+str(iter).zfill(5)+'_vol?????.vol')
                for volume in volumes:
                    self.ShowVolumes.append(volume)

    def visualize_projmatch_overall(self):
    ...
    def visualize_matrix_last_iter(self):
        import os,glob
        selfiles=glob.glob('Runprojmatch/projmatch_it?????.sel')
        if len(selfiles)==0:
            print "No selfiles yet. Visualize only after job completion..."
        else:
            lastselfile=selfiles[-1]
        selfile1='Runprojmatch/projmatch_lib.sel'
        selfile2=lastselfile
        self.join_selfiles(selfile1,selfile2,'matrixview_projmatch_lastiter.sel')
        self.ShowSelfiles.append('matrixview_projmatch_lastiter.sel')
            

    def show_statistics_alliter(self):
        import os,glob
        logfiles=glob.glob('Runprojmatch/projmatch_it?????.log')
        if len(logfiles)==0:
    ...
    def show_moving_particles(self):
        import os,glob
        docfiles=glob.glob('Runprojmatch/projmatch_it?????.doc')
