...
void Prog_MLalign3D_prm::produce_Side_info()
{
    ...
    for (double tilt=tilt0; tilt<=tiltF; tilt+=tilt_step)
    {
        ...
        if (tilt==0)
            ...
            nr_rot_tilt+=CEIL(360.*sin(DEG2RAD(tilt))/rot_step);
    }
    nr_psi=0;
    ...
    SF.go_beginning();
    vol.read(SF.NextImg());
    if (XSIZE(vol())!=YSIZE(vol()))
        REPORT_ERROR(ERR_MULTIDIM_SIZE,"Unequal dimensions: Only cubic volumes are allowed!");
    if (XSIZE(vol())!=ZSIZE(vol()))
        REPORT_ERROR(ERR_MULTIDIM_SIZE,"Unequal dimensions: Only cubic volumes are allowed!");
    dim=XSIZE(vol());
    ...
    if (do_subtract_current && nr_ref > 1)
        REPORT_ERROR(ERR_ARG_INCORRECT,"subtract_current for now only implemented with 1 reference!");
    if (do_subtract_current && !ccf_mode)
        REPORT_ERROR(ERR_ARG_INCORRECT,"ERROR% subtract_current for now only implemented for -CCF!");
    if (fn_mask!="")
    {
        if (!ccf_mode)
            REPORT_ERROR(ERR_ARG_INCORRECT,"Mask option for now only implemented for -CCF!");
        else
        {
    ...
    double &opt_xoff, double &opt_yoff, double &opt_zoff)
{
    ...
    FOR_ALL_TILT()
    {
        ...
        if (tilt==0)
            ...
            nr_rot=CEIL(360.*sin(DEG2RAD(tilt))/rot_step);
        ...
        FOR_ALL_ROT()
        {
            ...
            FOR_ALL_PSI()
            {
                ...
                FOR_ALL_MODELS()
                {
                    ...
                    if (verb>0)
                        std::cout <<rot<<" "<<tilt<<" "<<psi<<" "<<xmax<<" "<<ymax<<" "<<zmax<<" "<<max<<std::endl;
                    if (max>maxccf)
                    {
                        maxccf=max;
                    ...
                    if (alpha_k[refno]>0.)
                    {
                        ...
                        FOR_ALL_ELEMENTS_IN_MATRIX3D(Mweight[iweight])
                        {
                            ...
                            if (aux>1000.)
                                ...
                                aux=exp(-aux)*alpha_k[refno]*VOL_ELEM(pdf_trans,k,i,j);
        ...
        double &wsum_sigma_offset, std::vector<double> &sumw)
{
    ...
    while ((!SF.eof()))
    {
        ...
        if (nr_wedge>0)
            dataline(6)=img_wednr[imgno];        // missing wedge number
        else
            dataline(6)=0.;
        ...
        double &sumcorr, double &sumw_allrefs, int iter)
{
    ...
    {
        if (fn_solv!="")
            solvent_flattening(fn_solv);
        if (fn_solv2!="")
            solvent_flattening(fn_solv2);
    }
    ...
    {
        if (theta==0)
            sigma_noise2=wsum_sigma_noise2/(sumw_allrefs*dim3);
        else
        {
            double diff2_ref=0.;
            FOR_ALL_MODELS()
        ...
        double &LL, double &avecorr)
{
    ...
    if (theta>0)
        comment+=" -theta "+floatToString(theta,6,3)+" -theta_step "+floatToString(theta_step,6,3);
    DF.insert_comment(comment);
