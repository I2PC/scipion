...
void QtMainWidgetMark::add_point(const Particle_coords &U,
   ...
   __Au(2,0) =__Au(0,2); __Au(2,1) =__Au(1,2); __Au(2,2) =__Nu;

   // Adjust tilted dependent matrix
   __Bt(0,0)+=T.X*U.X  ; __Bt(0,1)+=T.Y*U.X  ; __Bt(0,2) =__Au(0,2);
   __Bt(1,0)+=T.X*U.Y  ; __Bt(1,1)+=T.Y*U.Y  ; __Bt(1,2) =__Au(1,2);
...
void QtMainWidgetMark::compute_alphas() {
   ...
   matrix1D<double> angles(2); angles.init_zeros();
   matrix1D<double> steps(2); steps.init_constant(1);
   double fitness;
   int iter;
   pair_Put=&__Put;
   pair_tilt=__gamma;
   ...
   for (aux[1]=0; aux[1]<180; aux[1]+=10)
      for (aux[2]=0; aux[2]<180; aux[2]+=10) {
         ...
         if (fit<best_fit)
	    {best_fit=fit; best_alpha_u=aux[1]; best_alpha_t=aux[2];}
      }
