...
double Classification_model::distance(const matrix1D<double> &X,
   ...
   matrix2D<double> dif; dif.from_vector(X-Y);
   double dist2;
...
void QtWidgetMicrograph::classifyMask() {
   ...
   classif1->copy_shape(mask); classif1->init_constant(-1);
...
bool QtWidgetMicrograph::build_vector(int _x, int _y,
   ...
   if (FINISHINGY(mask)+_y>FINISHINGY(__piece)) return false;
      ...
      if (true) {
	 save()=__piece; save.write("PPP0.xmp");
   ...
   for (int i=0;i<imax;i++)
       for (int j=0; j<jmax; j++)
	      ...
	      <__min_distance_between_particles) {
	         _candidate_vec.at(i).status=0; break;
	      }
}
...
void QtWidgetMicrograph::refine_center(Particle &my_P) {
   ...
   do {
	 ...
	 if (success) {
	    ...
	    if (dist_right<best_dist) {best_dist=dist_right; source=1;}
	 }
      }
	 ...
	 if (success) {
	    ...
	    if (dist_top<best_dist) {best_dist=dist_top; source=2;}
	 }
      }
	 ...
	 if (success) {
	    ...
	    if (dist_bottom<best_dist) {best_dist=dist_bottom; source=3;}
	 }
      }
      ...
      if (source==-1) {
         ...
         switch (source) {
	    case 0: current_x--; previous_direction=1; break;
	    case 1: current_x++; previous_direction=0; break;
	    case 2: current_y--; previous_direction=3; break;
	    case 3: current_y++; previous_direction=2; break;
	 }
...
void QtWidgetMicrograph::write() _THROW {
   ...
   // Save the mask
   ImageXmipp save; type_cast(__mask.get_binary_mask2D(),save());
