...
bool process_img(FourierImageXmipp &IMG, ImageXmipp &img,
   ...
   xmippFFT2VTK(IMG,vtk);
   IFFT_VTK(vtk,img(),TRUE);
   vtk->Delete();
   return TRUE;
}
...
bool process_vol(VolumeXmipp &vol, FourierVolumeXmipp &VOL,
   const Prog_parameters *prm) {
   vtkImageData *vtk=NULL;
   xmippFFT2VTK(VOL,vtk);
   IFFT_VTK(vtk,vol(),TRUE);
   vtk->Delete();
   return TRUE;
}
...
/*Colimate:
   PROGRAM Inverse_FFT {
      url="http://www.cnb.uam.es/~bioinfo/NewXmipp/Applications/Src/IFFT/Help/IFFT.html";
      help="Inverse Fourier Transform of volumes and images";
      OPEN MENU menu_IFFT;
      COMMAND LINES {
	+ usual: xmipp_IFFT
