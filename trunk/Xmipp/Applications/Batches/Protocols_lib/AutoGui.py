...
class automated_gui_class:
    ...
    def ScriptWrite(self):
        ...
        for i in range(len(self.script_header_lines)):
            if (self.script_header_lines[i][0]!="#"):
...
#                if ( "/" in args[1] ):
#                    self.variables[args[0]].append("FileName")
#                    newvar=StringVar()
#                    self.variables[args[0]].append(newvar)
                else:
                ...
                comment,isexpert=self.parse_comment_line(self.script_header_lines[i-1])
                self.variables[args[0]].append(comment)
                self.variables[args[0]].append(isexpert)

    def MasterGuiFill(self):
    ...
    def GuiFill(self):
        ...
        # Script title
        headertext="Xmipp Protocol GUI for "+str(self.scriptname)
        self.GuiAddSection(headertext,1)
    ...
    def GuiAddRestButtons(self):
        ...
        self.hi_there = Button(self.frame, text="Help", command=self.GuiHelp)
