...
/* Read a SOM -------------------------------------------------------------- */
void ShowSOM::readFile(const FileName &_fn_root) _THROW {
    clear();
    fn              = _fn_root;
    setCaption(fn.c_str());
...
/* Change labels ----------------------------------------------------------- */
void ShowSOM::changeLabelsToImg() {changeLabel(mi_imgAsLabels);}
void ShowSOM::changeLabelsToSel() {changeLabel(mi_selAsLabels);}
void ShowSOM::changeLabelsToHis() {changeLabel(mi_hisAsLabels);}
void ShowSOM::changeLabelsToErr() {changeLabel(mi_errAsLabels);}
...
void ShowSOM::showErrorImage() {  
    ...
    if (SFNew.ImgNo()) {
       ...
       ImageXmipp image(imgnames[i]), error_image;
