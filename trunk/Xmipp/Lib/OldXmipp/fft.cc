...
/***************************************************************************
 ...
 fftn.c, the routines that perform the fft of data with dimensions that can
 not be a power of 2
...
void fftcol(float **x,int col1,float **y,int col2,
            int n,int m,int sentido)
{
    register int i0, i1;
    ...
    /* float **x : Real part array and column position */
    /* float **y : Imaginary part array and column position */
...
/****************************************************************************/
/* This routine performs a direct FFT algorithm on the array volumen (4 byte*/
/* /voxel volum) and leaves the result in the same array. Intended ONLY for */
/* real volumes. Only half of the slices are transformed (conj. symm. prop.)*/
/* Returns ERROR if the dimensions are not an integral power of two,        */
/* OK if everything has worked as planned                                   */
/****************************************************************************/
...
if (no_correct (sli, &poweri))
    return ERROR;
if (no_correct (row, &powerj))
    return ERROR;
if (no_correct (col, &powerk))
    return ERROR;
...
if ((y = (float *) malloc ((max_dim+2)*sizeof(float))) == NULL)
{   puts ("Error: no hay memoria (fft)");
    exit (1);
}
...
switch (kind) {
    case DIRECT:                         /********** direct FFT **********/
    /****** 1st, Fourier transform by  slices ********/
        for (j=0; j < row; j++)
            for (k=0; k < col; k++)
            {   for (i=0; i < sli; i++)
                     x[i] = vol[i][j][k];
    ...
    /****** 2nd, Fourier transform by  rows   ********/
        for (i=0; i <= sli/2; i++)
            ...
            {   for (j=0; j < row; j++)
                ...
                cfft (x, y, row, powerj, 0);
                for (j=0; j < row; j++)
                {    vol[2*i][j][k] = x[j];
                     vol[2*i+1][j][k] = y[j];
                }
            }
    /****** 3rd, Fourier transform by  columns *******/
        for (i=0; i <= sli/2; i++)
            for (j=0; j < row; j++)
            {   for (k=0; k < col; k++)
                {    x[k] = vol[2*i][j][k];
            ...
            break;
    case INVERSE:                        /********** direct FFT **********/
        divisor = (float) row * (float) col;
    /****** 1st, Fourier transform by  columns *******/
        for (i=0; i <= sli/2; i++)
            for (j=0; j < row; j++)
            {   for (k=0; k < col; k++)
                {    x[k] = vol[2*i][j][k];
    ...
    /****** 2nd, Fourier transform by  rows   ********/
        for (i=0; i <= sli/2; i++)
            ...
            {   for (j=0; j < row; j++)
                ...
                cfft (x, y, row, powerj, 1);
                for (j=0; j < row; j++)
                {    vol[2*i][j][k] = x[j];
                     vol[2*i+1][j][k] = y[j];
                }
            }
    /****** 3rd, Fourier transform by  slices ********/
        for (j=0; j < row; j++)
            for (k=0; k < col; k++)
            {   for (i=0; i < sli+2; i++)
                     x[i] = vol[i][j][k];
                rfft (x, sli, poweri, 1);
                for (i=0; i < sli; i++)
                     vol[i][j][k] = x[i]/divisor; /*** Re-scale output ***/
            }
            break;
}
free ((void *) x);
