...
void Prog_parameters::show() {
   ...
   if (each_image_produces_an_output) {
      ...
      if (oroot!="")  cout << "Output Root: " << oroot << endl;
   }
}
...
try {
   ...
   // For single volume ....................................................
      ...
      vol.read(prm->fn_in); vol().set_Xmipp_origin();
      switch (operation_mode) {
      	 case IMAGE2IMAGE:
	    fv2v=(bool (*) (VolumeXmipp &, const Prog_parameters *)) process_vol;
            success=fv2v(vol, prm);
	    if (prm->each_image_produces_an_output) vol.write(fn_out);
	    break;
	 case IMAGE2FOURIER:
   ...
   // For a selection file .................................................
      ...
      while (!SF_in.eof()) {
         ...
         if (prm->each_image_produces_an_output)
            if (prm->oext=="" && prm->oroot=="") prm->fn_out=fn_read;
            else if (prm->oroot!="") {
	 ...
	 if (Is_ImageXmipp(fn_read)) {
	   ...
	   switch (operation_mode) {
	    ...
	    vol.read(fn_read); vol().set_Xmipp_origin();
	    switch (operation_mode) {
      	       case IMAGE2IMAGE:
		  fv2v=(bool (*) (VolumeXmipp &, const Prog_parameters *)) process_vol;
        	  fv2v(vol, prm);
	    ...
	    switch (operation_mode) {
         ...
         if (prm->each_image_produces_an_output)
            if (success) SF_out.insert(prm->fn_out);
            else         SF_out.insert(prm->fn_out, SelLine::DISCARDED);

         if (i++%istep==0 && prm->allow_time_bar) progress_bar(i);
      }
      if (prm->allow_time_bar) progress_bar(SF_in.ImgNo());
      if (prm->each_image_produces_an_output)
         if (prm->oext!="") {
