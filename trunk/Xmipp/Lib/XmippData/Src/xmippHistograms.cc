...
void histogram1D::insert_value(double val) {
   int i;
   val2index(val,i);
   if (i==-1) return; // the value is outside our scope
   VEC_ELEM(*this,i)++;
   no_samples++;
   #ifdef DEBUG
...
double histogram1D::mass_below(double value) {
   // Trivial cases
   if (value<=hmin) return 0;
   if (value>=hmax) return no_samples;
...
double detectability_error(const histogram1D &h1, const histogram1D &h2) {
   ...
   int N=0;
   while (v<=hmax) {
      h1.val2index(v,ih1); p1=VEC_ELEM(h1,ih1)/h1.no_samples;
      h2.val2index(v,ih2); p2=VEC_ELEM(h2,ih2)/h2.no_samples;
      //#define DEBUG
      #ifdef DEBUG
...
void histogram2D::insert_value(double v, double u) {
   int i, j;
   val2index(v, u, i, j);
   if (i==-1 || j==-1) return; // it is outside our scope
