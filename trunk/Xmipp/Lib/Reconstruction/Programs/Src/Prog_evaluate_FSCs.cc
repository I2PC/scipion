...
ostream & operator << (ostream &out, const Prog_Evaluate_FSCs_Parameters &prm) {
   ...
   out << "Action          : ";
   switch (prm.action) {
      case ESTIMATE_SINGLE_FSC:         out << "Estimate single FSC\n"; break;
      case ESTIMATE_AVERAGE_RESOLUTION: out << "Estimate average resolution\n"; break;
      case ESTIMATE_AVERAGE_FSC:        out << "Estimate average FSC\n"; break;
      case COMPARE_TWO_SETS:            out << "Compare two sets: "
                                            << prm.fn_recons2 << endl; break;
   }
   return out;
}
...
void Prog_Evaluate_FSCs_Parameters::compute_average_FSC(
   ...
   while (!SF_recons.eof()) {
      reconstruction.read(SF_recons.NextImg());
      compute_FSC(phantom,reconstruction, sampling_rate, frequency, FSC);
      if (n==0) {avg_FSC.init_zeros(FSC); max_FSC=min_FSC=FSC;}
      avg_FSC   +=FSC;
