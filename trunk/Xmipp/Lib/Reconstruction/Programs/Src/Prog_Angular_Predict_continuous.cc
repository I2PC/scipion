...
#include "../Prog_Angular_Predict_continuous.hh"
#include <XmippData/Bilib/configs.h>
#include <XmippData/Bilib/headers/linearalgebra.h>
#include <XmippData/Bilib/headers/error.h>
#include <XmippData/Bilib/headers/messagedisplay.h>
#include <XmippData/Bilib/headers/getputd.h>
#include <XmippData/Bilib/headers/kernel.h>
#include <XmippData/Bilib/headers/kerneldiff1.h>
#include <XmippData/Bilib/types/tsplinebasis.h>
#include <XmippData/Bilib/types/tboundaryconvention.h>
#include <XmippData/Bilib/headers/changebasis.h>
#include <XmippData/Bilib/headers/dft.h>
   ...
   double *dftCompProj)
{
   ...
   *hlp = - Sintheta;
   ...
   for (indx = 0L; indx < Mx; indx++){
      ...
      slices_d3 = 0.0;
      for (n = n1; n <= n2; n++){
         CC1 = Nx * Ny * periodization(Nz, n - indz0);
         columns = 0.0;
         columns_d1 = 0.0;
         columns_d2 = 0.0;
         for (m = m1; m <= m2; m++){
            CC = CC1 + Nx * periodization(Ny, m - indy0);
            rows = 0.0;
            rows_d1 = 0.0;
            for (l = l1; l <= l2; l++){
               xminusl = x - (double) l;
               Coeff = (double) CoefRe[CC + periodization(Nx, l - indx0)];
               rows += Coeff * Bspline03(xminusl);
               rows_d1 += Coeff * Bspline03Diff1(xminusl);         
            }
            yminusm = y - (double) m;
            columns +=  rows * Bspline03(yminusm);
            columns_d1 +=  rows_d1 * Bspline03(yminusm);
            columns_d2 +=  rows * Bspline03Diff1(yminusm);
         }
         zminusn = z - (double) n;
         slices +=  columns * Bspline03(zminusn);
         slices_d1 +=  columns_d1 * Bspline03(zminusn);
         slices_d2 +=  columns_d2 * Bspline03(zminusn);
         slices_d3 +=  columns * Bspline03Diff1(zminusn);
      }
      ...
      for (n = n1; n <= n2; n++){
         ...
         for (m = m1; m <= m2; m++){
            ...
            for (l = l1; l <= l2; l++){
               xminusl = x - (double) l;
               Coeff = (double) CoefIm[CC + periodization(Nx, l - indx0)];
               rows += Coeff * Bspline03(xminusl);
               rows_d1 += Coeff * Bspline03Diff1(xminusl);         
            }
            yminusm = y - (double) m;
