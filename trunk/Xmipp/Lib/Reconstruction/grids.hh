...
public:
   ...
   void get_X(matrix1D<double> &v) const { basis.getCol(0,v);}

   /** Get Y vector of the grid.
       \\Ex: matrix1D<double> Y; sg.get_Y(Y) << endl; */
   void get_Y(matrix1D<double> &v) const { basis.getCol(1,v);}

   /** Get Z vector of the grid.
       \\Ex: matrix1D<double> Z; sg.get_Z(Z) << endl; */
   void get_Z(matrix1D<double> &v) const { basis.getCol(2,v);}
   ...
   */
   ...
   void Gproject_to_plane(const matrix1D<double> &gr,
      ...
      {grid2universe(gr, result); Uproject_to_plane(result,euler,result);}
   ...
   void Gdir_project_to_plane(const matrix1D<double> &gr,
      double rot, double tilt, double psi, matrix1D<double> &result) const
      {grid2universe(gr,result); V3_MINUS_V3(result, result, origin);
...
/* Protoypes --------------------------------------------------------------- */
   ...
   int GridsNo() const {return LG.size();}
   ...
   template <class T1>
      void resize(const GridVolumeT<T1> &GV) {
         ...
         for (int n=0; n<GV.VolumesNo(); n++) {
            SimpleGrid grid; grid=GV.grid(n); G.add_grid(grid);

            VolumeT<T> *Vol_aux; Vol_aux = new VolumeT<T>;
            (*Vol_aux)().resize(GV(n)());
