...
if [ $DO_PYTHON -eq 1 ]; then
  if [ $DO_CLEAN -eq 1 ]; then
    echoExec "rm ${XMIPP_HOME}/lib/libpython2.7.so" "/dev/null"
    echoExec "rm ${XMIPP_HOME}/lib/libpython2.7.so.1.0" "/dev/null"
  fi
...
#    printf 'source ${XMIPP_HOME}/bin/activate' >> $PYTHON_BIN
  ...
  echoExec "ln -s ${XMIPP_HOME}/external/python/Python-2.7.2/libpython2.7.so ${XMIPP_HOME}/lib/libpython2.7.so" "/dev/null"
  echoExec "ln -s ${XMIPP_HOME}/external/python/Python-2.7.2/libpython2.7.so.1.0 ${XMIPP_HOME}/lib/libpython2.7.so.1.0" "/dev/null"
