...
public class MetadataTest {
	...
	String[] imageValues = { "000001@tux.stk", "000002@tux.stk",
			"000003@tux.stk", "000004@tux.stk" };
	...
	public void testRead() throws Exception {
		...
		for (int i = 0; i < ids.length; ++i) {
			id = ids[i];
			assertEquals(imageValues[i],
					md.getValueString(MDLabel.MDL_IMAGE, id));
			assertEquals(shiftXValues[i],
					md.getValueDouble(MDLabel.MDL_SHIFT_X, id),
					XmippTest.EQUAL_ACCURACY);
	...
	public void testSort() throws Exception {
		...
		long[] ids = md.findObjects();
		for (int i = 0; i < ids.length; ++i)
			assertEquals(shiftXSorted[i],
					md.getValueDouble(MDLabel.MDL_SHIFT_X, ids[i]),
					XmippTest.EQUAL_ACCURACY);
	...
	public void testFill() throws Exception {
		...
		for (int i = 0; i < ids.length; ++i)
			assertEquals(imagePath,
					md.getValueString(MDLabel.MDL_IMAGE1, ids[i]));

		// Test fillLinear
		md.fillLinear(MDLabel.MDL_SHIFT_Y, 0.0, 0.5);
		double value = 0.0;
		for (int i = 0; i < ids.length; ++i) {
			assertEquals(value, md.getValueDouble(MDLabel.MDL_SHIFT_Y, ids[i]),
					XmippTest.EQUAL_ACCURACY);
			value += 0.5;
		}
		// md.print();
	...
	public void testRemoveObject() throws Exception {
		...
		ids = md.findObjects();
		for (int i = 0; i < ids.length; ++i)
			assertEquals(localShiftXValues[i],
					md.getValueDouble(MDLabel.MDL_SHIFT_X, ids[i]),
					XmippTest.EQUAL_ACCURACY);
		// md.print();
		md.destroy();
