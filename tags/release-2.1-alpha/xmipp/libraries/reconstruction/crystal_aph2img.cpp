...
void Spot2RealSpace2D_Parameters::read_from_file(const FileName &fnprm)
{
    ...
    {
#ifdef REMOVE
        fnaph_ref = getParameter(fh_param, "APH Ref file", 0, "",
        ...
        try
        {
            *is >> XX(Phase_Shift) >> YY(Phase_Shift);
        }
        catch (...)
        {
            REPORT_ERROR(3007,
...
void Spot2RealSpace2D_Parameters::produce_SideInfo()
{
    ...
    if (str_symmetry_group == "P1")
    {
        symmetry_group = sym_P1;
        a_b_ang = 0;
    }//not sure about this
...
#ifdef DEBUG
    ImageXmipp save;
    save() = aph_file.spots_abs;
    save.write("Spots_ABS0.xmp");
    save() = aph_file.spots_arg;
    save.write("Spots_ARG0.xmp");
#endif
...
void RealSpace2Spots2D_Parameters::read_from_file(const FileName &fnprm)
{
    ...
    try
    {
        fn_in = getParameter(fh_param, "INput file", 0, NULL,
