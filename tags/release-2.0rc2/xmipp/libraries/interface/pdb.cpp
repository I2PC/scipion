    ...
    const Matrix2D<double> &A, bool centerPDB)
{
	...
	char aux[15];
	sprintf(aux,"%8.3f",XX(v)); line.replace(30,8,aux);
	sprintf(aux,"%8.3f",YY(v)); line.replace(38,8,aux);
	sprintf(aux,"%8.3f",ZZ(v)); line.replace(46,8,aux);
    ...
    Matrix1D<double> &projectionProfile)
{
	...
	if (atomValue.r0_2>r2) continue; // Because of numerical instabilities
