                           ...
                           const int mrc_label)
{
    ...
    while (!fh_aph.eof())
    {
        try
        {
            ...
            h = tmp_spot.h       = textToInteger(firstToken(line));
            k = tmp_spot.k       = textToInteger(nextToken());
            tmp_spot.zstar       = textToFloat(nextToken());
            tmp_spot.amp       = textToFloat(nextToken());
            tmp_spot.phase       = textToFloat(nextToken());
            FILM = tmp_spot.FILM  = textToInteger(nextToken());
            tmp_spot.IQ       = textToInteger(nextToken());
            tmp_spot.FLMWGT      = textToFloat(nextToken());
            tmp_spot.BACK       = textToFloat(nextToken());
            tmp_spot.myCTF       = textToFloat(nextToken());
...
#undef DEBUGTILTZERO
         ...
         line_no >= 0; line_no--)
    {
        ...
        if ((aph_data_vector[line_no]).h > 0)
        {
            second.phase *= -1.;
            second.zstar *= -1.;
            third.phase   = -1. * third.phase + 180.0 * third.k;
            third.k  *= -1;
...
#undef DEBUG
        ...
        diff_z[3] = (ABS(fourth.zstar - (fourth.h * h_contrib +
                                         fourth.k * k_contrib)));
        minimun_diff = diff_z[0];
        aux_spot = first;
        if (minimun_diff > diff_z[1])
        {
            aux_spot = second;
            minimun_diff = diff_z[1];
        }
        if (minimun_diff > diff_z[2])
        {
            aux_spot = third;
            minimun_diff = diff_z[2];
        }
        if (minimun_diff > diff_z[3])
        {
            aux_spot = fourth;
