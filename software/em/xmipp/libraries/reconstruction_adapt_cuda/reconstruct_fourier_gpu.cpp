...
inline void ProgRecFourierGPU::getVectors(const Point3D* plane, Point3D& u, Point3D& v) {
	float x0 = plane[0].x;
	float y0 = plane[0].y;
	float z0 = plane[0].z;
	u.x = plane[1].x - x0;
...
inline void ProgRecFourierGPU::multiply(const float transform[3][3], Point3D& inOut) {
...
void ProgRecFourierGPU::createProjectionCuboid(Point3D* cuboid, float sizeX, float sizeY, float blobSize)
{
...
inline void ProgRecFourierGPU::translateCuboid(Point3D* cuboid, Point3D vector) {
	for (int i = 0; i < 8; i++) {
		cuboid[i].x += vector.x;
		cuboid[i].y += vector.y;
		cuboid[i].z += vector.z;
...
void ProgRecFourierGPU::computeAABB(Point3D* AABB, Point3D* cuboid,
...
//	d->localAInv = Matrix2D<double>(3, 3);
//	d->localAInv(0, 0) = 0;
//	d->localAInv(0, 1) = 1;
//	d->localAInv(0, 2) = 2;
//	d->localAInv(1, 0) = 10;
...
//	d->localAInv(2, 2) = 22;
//
//	d->img = new Array2D<std::complex<float> >(20, 1);
//	for (int i = 0; i < 20; i++) {
//		(*d->img)(i, 0) = i;
//		std::cout << (*d->img)(i, 0) << " ";
//	}
