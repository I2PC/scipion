 ...
 ***************************************************************************/
#ifndef _PROG_GPU_ROTATE_IMAGE_PARALLEL
#define _PROG_GPU_ROTATE_IMAGE_PARALLEL
...
#include "../reconstruction_cuda/cuda_gpu_rotate_image_linear_nearest_parallel.h"
...
void transformationMatrixCudaParallel(const MDRow &imageGeo, Matrix2D<double> &A, bool isVol);

//CUDA functions
float** cuda_rotate_image_parallel(float **image, float **rotated_image, size_t *Xdim,
		size_t *Ydim, size_t *Zdim, size_t mdInSize, double** transform_matrix, int interp, int wrap, int first_call);


class ProgGpuRotateImageParallel: public XmippProgram
{
private:
    //Input file
	FileName fn_sel;
