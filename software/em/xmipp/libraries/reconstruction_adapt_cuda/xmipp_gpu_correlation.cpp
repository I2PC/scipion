		...
		mycufftHandle &myhandlePadded, mycufftHandle &myhandleMask, mycufftHandle &myhandlePolar, mycufftHandle &myhandleAux)
{
	...
	padding_masking(image_stack_gpu, d_correlationAux.d_mask, padded_image_gpu, padded_image2_gpu,
	...
	GpuMultidimArrayAtGpu<float> polar_gpu(360,radius,1,numImages);
	GpuMultidimArrayAtGpu<float> polar2_gpu(360,radius,1,numImages);
...
void preprocess_images_experimental(MetaData &SF, FileName &fnImg, int numImagesRef, GpuMultidimArrayAtGpu<float> &mask,
		...
		int firstStep, bool mirror, mycufftHandle &myhandlePadded, mycufftHandle &myhandleMask, mycufftHandle &myhandlePolar, double _radius)
{
	size_t Xdim, Ydim, Zdim, Ndim;
	getImageSize(SF,Xdim,Ydim,Zdim,Ndim);
	size_t pad_Xdim=2*Xdim-1;
	size_t pad_Ydim=2*Ydim-1;
	size_t radius=(size_t)_radius;
	...
	if(!rotation){
		...
		padding_masking(image_stack_gpu, mask, padded_image_gpu, padded_image2_gpu,
	...
	size_t nAux1= d_correlationAux.d_original_image.Ndim;
...
void preprocess_images_experimental_transform(GpuCorrelationAux &d_correlationAux, GpuMultidimArrayAtGpu<float> &mask,
		...
		mycufftHandle &myhandleMask, mycufftHandle &myhandlePolar, double _radius)
{
	...
	if(!rotation){
		...
		padding_masking(d_correlationAux.d_transform_image, mask, padded_image_gpu, padded_image2_gpu,
				padded_mask_gpu, rotation, true);
...
void align_experimental_image(FileName &fnImgExp, GpuCorrelationAux &d_referenceAux, GpuCorrelationAux &d_experimentalAux,
		...
		mycufftHandle &myhandlePaddedB, mycufftHandle &myhandleMaskB, mycufftHandle &myhandlePolarB, double radius)
{
	...
	for(int firstStep=0; firstStep<2; firstStep++){ //2
		...
		preprocess_images_experimental(SFexp, fnImgExp, available_images_proj, d_referenceAux.d_mask, d_referenceAux.d_maskFFT,
				d_experimentalAux, rotation, firstStep, mirror, myhandlePadded, myhandleMask, myhandlePolar, radius);

		if(!rotation){
			d_experimentalAux.maskCount=d_referenceAux.maskCount;
			d_experimentalAux.produceSideInfo(myhandlePaddedB, myhandleMaskB, d_referenceAux.maskAutocorrelation);
		}
		...
		for(int step=0; step<max_step; step++){ //loop over consecutive translations and rotations (TRTRTR or RTRTRT) 6
			...
			if(step<max_step-1){
				...
				preprocess_images_experimental_transform(d_experimentalAux, d_referenceAux.d_mask, d_referenceAux.d_maskFFT, rotation, step,
						myhandlePadded, myhandleMask, myhandlePolar, radius);
...
void ProgGpuCorrelation::run()
{
	...
	float *max_vector_rt = new float [available_images_proj];
	float *max_vector_tr = new float [available_images_proj];
	float *max_vector_rt_mirror = new float [available_images_proj];
	float *max_vector_tr_mirror = new float [available_images_proj];
	...
	while(!finish){
		...
		while(available_images_exp && iterExp->objId!=0){
			...
			align_experimental_image(fnImgExp, d_referenceAux, d_experimentalAux, transMat_tr, transMat_rt,
					...
					myhandlePadded, myhandleMask, myhandlePolar, myhandlePaddedB, myhandleMaskB, myhandlePolarB, mask.R1);
