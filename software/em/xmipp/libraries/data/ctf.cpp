...
void CTFDescription::lookFor(int n, const Matrix1D<double> &u, Matrix1D<double> &freq, int iwhat)
{
    ...
    {
    	std::cout << "freq =" << XX(freq) << "u =" << u << "w =" << w << std::endl;
        V2_BY_CT(freq, u, w);
        std::cout << "XX(freq) =" << XX(freq) << "YY(freq)" << YY(freq) << "u =" << u << "w =" << w << std::endl;
...
/* Read -------------------------------------------------------------------- */
void CTF1D::readFromMdRow(const MDRow &row, bool disable_if_not_K)
{
    row.getValueOrDefault(MDL_CTF_SAMPLING_RATE, Tm, 1);
...
void CTF1D::readFromMetadataRow(const MetaData &md, size_t id, bool disable_if_not_K)
{
    MDRow row;
    md.getRow(row, id);
    readFromMdRow(row, disable_if_not_K);
...
void CTF1D::read(const FileName &fn, bool disable_if_not_K)
{
	if (fn.isMetaData())
	{
		MetaData md;
...
/* Write ------------------------------------------------------------------- */
void CTF1D::write(const FileName &fn)
{
	MDRow row;
	row.setValue(MDL_CTF_SAMPLING_RATE, Tm);
	if (enable_CTF)
...
/* Define Params ------------------------------------------------------------------- */
void CTF1D::defineParams(XmippProgram * program)
{
	program->addParamsLine("== CTF1D description");
	program->addParamsLine("  [--ctf_similar_to_fast++ <ctfFile>]        : ctfparam file");
	program->addParamsLine("                                        : Parameters from this file are ");
	program->addParamsLine("                                        : overriden by the parameters in the command line");
	program->addParamsLine("  [--sampling_rate_fast <Tm>]                : Angstroms/pixel. Ex: 1.4");
	program->addParamsLine("                                        : This parameter is compulsory if a CTF is needed.");
	program->addParamsLine("  [--voltage_fast <kV>]                      : Accelerating voltage (kV). Ex: 200");
	program->addParamsLine("                                        : This parameter is compulsory if a CTF is needed.");
	program->addParamsLine("     alias --kV_fast;");
	program->addParamsLine("  [--spherical_aberration_fast <Cs>]         : Milimiters. Ex: 5.6");
	program->addParamsLine("                                        : This parameter is compulsory if a CTF is needed.");
	program->addParamsLine("     alias --Cs_fast;");
	program->addParamsLine("  [--defocus <Defocus>]                : Defocus in Angstroms (Ex: 2000)");
	program->addParamsLine("  [--chromatic_aberration_fast++ <Ca=0>]     : Milimiters. Ex: 2");
	program->addParamsLine("  [--energy_loss_fast++ <espr=0>]            : eV. Ex: 1");
	program->addParamsLine("  [--lens_stability_fast++ <ispr=0>]         : ppm. Ex: 1");
	program->addParamsLine("  [--convergence_cone_fast++ <alpha=0>]      : mrad. Ex: 0.5");
	program->addParamsLine("  [--longitudinal_displace_fast++ <DeltaF=0>]: Angstrom. Ex: 100");
	program->addParamsLine("  [--transversal_displace_fast++ <DeltaR=0>] : Angstrom. Ex: 3");
	program->addParamsLine("  [--Q0_fast++ <Q0=0>]                       : Percentage of cosine (Q0>0)");
	program->addParamsLine("  [--K_fast++ <K=0>]                         : Global gain");
}

/* Read from command line -------------------------------------------------- */
void CTF1D::readParams(XmippProgram * program)
{
	kV=Tm=Cs=0;
	if (program->checkParam("--ctf_similar_to_fast"))
		read(program->getParam("--ctf_similar_to_fast"));
	if (program->checkParam("--sampling_rate_fast"))
		Tm=program->getDoubleParam("--sampling_rate_fast");
	if (Tm==0)
		REPORT_ERROR(ERR_ARG_MISSING,"--sampling_rate_fast");
	if (program->checkParam("--voltage_fast"))
		kV=program->getDoubleParam("--voltage_fast");
	if (kV==0)
		REPORT_ERROR(ERR_ARG_MISSING,"--voltage_fast");
	if (program->checkParam("--spherical_aberration_fast"))
		Cs=program->getDoubleParam("--spherical_aberration_fast");
	if (Cs==0)
		REPORT_ERROR(ERR_ARG_MISSING,"--spherical_aberration_fast");
	if (program->checkParam("--defocus"))
		Defocus=program->getDoubleParam("--defocus");
	if (program->checkParam("--Q0_fast"))
		Q0=program->getDoubleParam("--Q0_fast");
	Ca=program->getDoubleParam("--chromatic_aberration_fast");
	espr=program->getDoubleParam("--energy_loss_fast");
	ispr=program->getDoubleParam("--lens_stability_fast");
	alpha=program->getDoubleParam("--convergence_cone_fast");
	DeltaF=program->getDoubleParam("--longitudinal_displace_fast");
	DeltaR=program->getDoubleParam("--transversal_displace_fast");
	Q0=program->getDoubleParam("--Q0_fast");
	K=program->getDoubleParam("--K_fast");
}

/* Show -------------------------------------------------------------------- */
std::ostream & operator << (std::ostream &out, const CTF1D &ctf)
{
    if (ctf.enable_CTF)
    {
        out
...
/* Default values ---------------------------------------------------------- */
void CTF1D::clear()  // Esta parte es nueva
{
	enable_CTF = true;
	enable_CTFnoise = false;
	isLocalCTF = false;
...
/* Precompute values ------------------------------------------------------- */
void CTF1D::precomputeValues(const MultidimArray<double> &cont_x_freq)
{
	precomputedImage.reserve(MULTIDIM_SIZE(cont_x_freq));
	    precomputedImageXdim=XSIZE(cont_x_freq);
...
/* Look for zeroes, maxima or minima ------------------------------------------------------------ */
//#define DEBUG
void CTF1D::lookFor(int n, const Matrix1D<double> &u, Matrix1D<double> &freq, int iwhat)
{
    double wmax = 1 / (2 * Tm);
    double wstep = wmax / 300;
    int found = 0;
    ...
    for (w = 0; w <= wmax; w += wstep)
    {
        ...
        V2_BY_CT(freq, u, w);
        /*std::cout <<"freqb="<< freq << std::endl;
		std::cout <<"ub="<< u << std::endl;
		std::cout <<"wb="<< w << std::endl;*/
...
/* Apply the CTF to an image ----------------------------------------------- */
void CTF1D::applyCTF(MultidimArray < std::complex<double> > &FFTI, const MultidimArray<double> &I, double Ts, bool absPhase)
{
    Matrix1D<int>    idx(2);
    Matrix1D<double> freq(2);
    if ( ZSIZE(FFTI) > 1 )
...
void CTF1D::applyCTF(MultidimArray <double> &I, double Ts, bool absPhase)
{
	FourierTransformer transformer;
	MultidimArray<double> FFTI;
	transformer.setReal(I);
...
/* Get profiles ------------------------------------------------------------ */
void CTF1D::getProfile(double fmax, int nsamples,
                                MultidimArray<double> &profiles)
{
    double step = fmax / nsamples;
...
/* Get average profiles ----------------------------------------------------- */
void CTF1D::getAverageProfile(double fmax, int nsamples,
                                       MultidimArray<double> &profiles)
{
    double step = fmax / nsamples;
    profiles.initZeros(nsamples, 4);
...
/* Force Physical meaning -------------------------------------------------- */
void CTF1D::forcePhysicalMeaning()
{
    if (enable_CTF)
    {
        if (K < 0)
