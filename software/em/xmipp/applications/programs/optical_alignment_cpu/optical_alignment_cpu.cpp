...
public:
    ...
    MultidimArray<double> movieStack;
    ...
    {
        int N = (end-begin)+1;
        std::cout<<"The number of images is equal to"<<N<<std::endl;
        std::cout<<"The first frame is"<<begin<<std::endl;
        std::cout<<"The last frame is"<<end<<std::endl;
        MultidimArray<double> imgNormal, sumImg;

        imgNormal.initZeros(YSIZE(movieStack), XSIZE(movieStack));
        movieStack.getImage(begin, imgNormal, 0);
        sumImg=imgNormal;
        ...
        for (int i=begin;i<end;i++)
        {
            ...
            movieStack.getImage(i+1, imgNormal, 0);
            sumImg += imgNormal;
        }
    ...
    int main2()
    {
        ...
        //h = aDim.ydim;
        //w = aDim.xdim;
        imagenum = NSIZE(movieStack);
        h = YSIZE(movieStack);
        w = XSIZE(movieStack);
        cout<<"The number of frames is equal to"<<imagenum<<std::endl;
        ...
        if (lstFrame>(imagenum-1) || lstFrame==0)
            lstFrame = imagenum-1;
        imagenum = lstFrame-fstFrame+1;
        levelNum = sqrt(double(imagenum));
        std::cout<<"Before computing average"<<std::endl;
        computeAvg(fname, fstFrame, lstFrame, avgCurr);
        std::cout<<"After computing average"<<std::endl;
        ...
        if (psd)
        {
            ...
            std::cerr<<"Computing the raw FFT"<<std::endl<<cmd<<std::endl;
        ...
        while (div!=1)
        {
            ...
            for (int i=0;i<cnt;i++)
            {
                ...
                if (div==1)
                    movieStack.getImage(i, preImg, 0);
                else
                {
                    if (i==cnt-1)
