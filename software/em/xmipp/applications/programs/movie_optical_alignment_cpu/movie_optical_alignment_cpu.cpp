...
public:
    ...
    void xmipp2Opencv(const MultidimArray<float> &xmippArray, cv::Mat &opencvMat)
    {
        ...
        memcpy(&opencvMat.at<float>(0,0),&DIRECT_A2D_ELEM(xmippArray,0,0),MULTIDIM_SIZE(xmippArray)*sizeof(float));
    }
    ...
    void opencv2Xmipp(const cv::Mat &opencvMat, MultidimArray<float> &xmippArray)
    {
        ...
        memcpy(&DIRECT_A2D_ELEM(xmippArray,0,0),&opencvMat.at<float>(0,0),MULTIDIM_SIZE(xmippArray)*sizeof(float));
    }
    ...
    void run()
    {
        ...
        while (currentGroupSize!=finalGroupSize)
        {
            ...
            for (int currentGroup=0; currentGroup<numberOfGroups; currentGroup++)
            {
                ...
                if (numberOfGroups>2)
                {
                    FileName flowXFileName=fnmovieRoot+fnBaseOut.removeLastExtension()+formatString("flowx_%d_%d.yml",levelCounter-1,currentGroup/2);
                    readMat(flowXFileName.c_str(), flowCurrentGroup[0]);

                    FileName flowYFileName=fnmovieRoot+fnBaseOut.removeLastExtension()+formatString("flowy_%d_%d.yml",levelCounter-1,currentGroup/2); // ***
                ...
                {
					FileName flowXFileName=fnmovieRoot+fnBaseOut.removeLastExtension()+formatString("flowx_%d_%d.yml",levelCounter,currentGroup);
					FileName flowYFileName=fnmovieRoot+fnBaseOut.removeLastExtension()+formatString("flowy_%d_%d.yml",levelCounter,currentGroup);
