/* ----------------------------------------------------------------------------
	...
	[4]	P. Brigger, F. Muller, K. Illgner and M. Unser, "Centered pyramids,"
...
long int 	NyOut;
	...
	Tmp = (double *)malloc((size_t)(NxOut*NyIn*(long)sizeof(double)));
	if (Tmp == (double *)NULL) {
	...
	if (NyIn > 1L) {
		...
		free(OutBuffer);
	   	...
	   	memcpy( Out, Tmp, (size_t)(NxOut*(long)sizeof(double)));
...
long int 	NyOut;
	...
	if (NxIn > 1L) {
		...
		free(OutBuffer);
	   	...
	   	memcpy( Out, In, (size_t)(NxIn*NyIn*(long)sizeof(double)));
...
int BaseIndex;
	...
	BaseIndex = RowNb*Nx;
	for (i=0L; i<RowSize; i++) 
		Image[BaseIndex+i] = (double)Row[i];
}
